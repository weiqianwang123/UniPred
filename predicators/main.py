...
To make videos of test tasks:
    python3 predicators/main.py --env view_plan --approach oracle --seed 0 \
        --make_test_videos --num_test_tasks 10
...
assert os.environ.get("FD_EXEC_PATH") != "", \
        "Please set the environment variable export FD_EXEC_PATH= to the path of the " \
...
def main() -> None:
    ...
    # We will want to save the tasks for some domain, as they are time-consuming
    # to generate.
    perceiver = create_perceiver(CFG.perceiver)
    tasks_fname, _ = utils.create_task_filename_str()
    env_train_tasks = env.get_train_tasks()
    if os.path.exists(tasks_fname):
        logging.info(f"\n\nLOADED {len(env_train_tasks)} Tasks from {tasks_fname}")
    else:
        logging.info(f"\n\nGENERATED {len(env_train_tasks)} Tasks, saving to {tasks_fname}")
        os.makedirs(tasks_fname, exist_ok=True)
        for idx, task in enumerate(env_train_tasks):
    ...
    if approach.is_learning_based or CFG.force_create_dataset:
...
def _run_pipeline(env: BaseEnv,
    ...
    if cogman.is_learning_based:
        ...
        if CFG.load_approach:
            ...
            logging.info("Using {} device for training".format(CFG.device))
        ...
        if CFG.skip_until_cycle < 0:
            if CFG.debug:
                results = _run_testing(env, cogman, train_tasks=train_tasks)
            else:
...
def _run_testing(env: BaseEnv, cogman: CogMan, train_tasks=None) -> Metrics:
    tasks_fname, _ = utils.create_task_filename_str(train=False)
    logging.info("Running test tasks...")
    env_test_tasks = env.get_test_tasks()
    if os.path.exists(tasks_fname):
        logging.info(f"\n\nLOADED {len(env_test_tasks)} Tasks from {tasks_fname}")
    else:
        logging.info(f"\n\nGENERATED {len(env_test_tasks)} Tasks, saving to {tasks_fname}")
        os.makedirs(tasks_fname, exist_ok=True)
        for idx, task in enumerate(env_test_tasks):
            utils.task2json(task, os.path.join(tasks_fname, f'{idx}.json'))
    test_tasks = [
        task.replace_goal_with_alt_goal() for task in env_test_tasks
    ]
    num_found_policy = 0
    num_solved = 0
    ...
    for test_task_idx, env_task in enumerate(test_tasks):
        ...
        logging.info(f"Make video: {make_video}")
        logging.info(f"Task {test_task_idx+1} / {len(test_tasks)}: "
                     f"{log_message}")
        if make_video:
            assert monitor is not None
