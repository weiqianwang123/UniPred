...
from predicators.structs import NSRT, Action, Array, DummyOption, \
    ...
    ParameterizedOption, Predicate, Segment, SpotAction, State, STRIPSOperator, Task, \
    Type, Variable, VarToObjSub, Video, VLMPredicate, _GroundLDLRule, \
    _GroundNSRT, _GroundSTRIPSOperator, _Option, _TypedEntity, EnvironmentTask, \
    DummyPredicate
...
class SingletonParameterizedOption(ParameterizedOption):
    ...
    def __init__(
        ...
        def _initiable(state: State, memory: Dict, objects: Sequence[Object],
                       params: Array) -> bool:
            # Commented this line because our environment will have transition noise
...
def create_random_nsrt_policy(
        dummy_nsrt: _GroundNSRT,
    ...
    If no applicable option can be found, query the fallback policy.
    """
    sorted_nsrts = sorted(dummy_nsrt, key=lambda o: o.name)
    cur_option = DummyOption

    def _policy(state: State) -> Action:
        nonlocal cur_option
        if cur_option is DummyOption or cur_option.terminal(state):
            sample_nsrt = sample_applicable_ground_nsrt(state, sorted_nsrts, predicates, rng)
            if sample_nsrt is not None:
                # Sample an option.
                option = sample_nsrt.sample_option(state,
                                                goal=goal,
                                                rng=rng)
                cur_option = option
...
def create_dataset_filename_str(
        ...
        include_reward: Optional[bool] = False,
...
def create_task_filename_str(train=True) -> Tuple[str, str]:
    """Generate strings to be used for the filename for a dataset file that is
    about to be saved.
    ...
    if train:
        task_fname_template = (
            f"{CFG.env}__{CFG.offline_data_method}__{CFG.demonstrator}__"
            f"{regex}__{CFG.included_options}__{CFG.seed}" + suffix_str)
        task_fname = os.path.join(
            CFG.data_dir,
            task_fname_template.replace(regex, str(CFG.num_train_tasks)))
    else:
        task_fname_template = (
            f"{CFG.env}__"
            f"{regex}_indomain_{CFG.in_domain_test}_{CFG.seed}" + suffix_str)
        task_fname = os.path.join(
            CFG.data_dir,
            task_fname_template.replace(regex, str(CFG.num_test_tasks)))
    return task_fname, task_fname_template
...
def add_ground_atom_dataset(
        ground_atom_dataset: List[GroundAtomTrajectory],
        added_predicates: Set[Predicate]) -> List[GroundAtomTrajectory]:
    """Apply all predicates to all trajectories in the dataset."""
...
def load_ground_neural_atom_dataset(
        dataset_fname: str,
        trajectories: List[LowLevelTrajectory]) -> List[GroundAtomTrajectory]:
    """Load a previously-saved ground atom dataset.
    ...
    if os.path.exists(dataset_fname):
        # Load the ground atoms dataset.
        with open(dataset_fname, "rb") as f:
            ground_atom_dataset_atoms = cpkl.load(f)
        assert ground_atom_dataset_atoms is not None
        assert len(trajectories) == len(ground_atom_dataset_atoms)
        logging.info("\n\nLOADED GROUND ATOM DATASET")
...
def save_ground_neural_atom_dataset(ground_atom_dataset: List[GroundAtomTrajectory],
    ...
    with open(dataset_fname, "wb") as f:
        cpkl.dump(ground_atom_dataset_to_pkl, f)
...
def replace_nsrt_predicates(
        old_nsrts: Collection[NSRT],
        predicate_mapper: Dict[Predicate, Predicate]) -> List[NSRT]:
    """Replace some predicates in the given NSRTs with the new ones."""
    new_nsrts = []
    for nsrt in old_nsrts:
        new_preconditions = set()
        new_add_effects = set()
        new_delete_effects = set()
        # precond mapping
        for precond in list(nsrt.preconditions):
            if precond.predicate in predicate_mapper:
                if precond.predicate.arity == 0:
                    new_precond = LiftedAtom(predicate_mapper[precond.predicate], [])
                else:
                    var = precond.variables
                    new_precond = predicate_mapper[precond.predicate](var)
                new_preconditions.add(new_precond)
            else:
                new_preconditions.add(precond)
        # add effect mapping
        for add_effect in list(nsrt.add_effects):
            if add_effect.predicate in predicate_mapper:
                if add_effect.predicate.arity == 0:
                    new_add_effect = LiftedAtom(predicate_mapper[add_effect.predicate], [])
                else:
                    var = add_effect.variables
                    new_add_effect = predicate_mapper[add_effect.predicate](var)
                new_add_effects.add(new_add_effect)
            else:
                new_add_effects.add(add_effect)
        # delete effect mapping
        for del_effect in list(nsrt.delete_effects):
            if del_effect.predicate in predicate_mapper:
                if del_effect.predicate.arity == 0:
                    new_del_effect = LiftedAtom(predicate_mapper[del_effect.predicate], [])
                else:
                    var = del_effect.variables
                    new_del_effect = predicate_mapper[del_effect.predicate](var)
                new_delete_effects.add(new_del_effect)
            else:
                new_delete_effects.add(del_effect)
        new_nsrt = NSRT(name=nsrt.name, parameters=nsrt.parameters, preconditions=new_preconditions,
                            add_effects=new_add_effects, delete_effects=new_delete_effects,
                            ignore_effects=nsrt.ignore_effects, option=nsrt.option,
                            option_vars=nsrt.option_vars, _sampler=nsrt.sampler)
...
def create_pddl_domain(operators: Collection[NSRTOrSTRIPSOperator],
    ...
    # Case 2: type hierarchy.
        ...
        for t in sorted(types):
            if t.parent:
                if t.parent not in parent_to_children_types:
                    parent_to_children_types[t.parent] = [t]
                else:
...
def create_video_from_partial_refinements(
    ...
    if CFG.failure_video_mode == "longest_only":
        ...
        if partial_refinements[0] is not None:
            _, plan = max(partial_refinements, key=lambda x: len(x[1]))
        else:
            plan = partial_refinements[-1]
...
def save_video(outfile: str, video: Video, rw: list = []) -> None:
    """Save the video to video_dir/outfile."""
    outdir = CFG.video_dir
    os.makedirs(outdir, exist_ok=True)
    outpath = os.path.join(outdir, outfile)
    # add the reward floats on the images of video
    if len(rw) != 0:
        final_rw = sum(rw)
        rw.append(final_rw) 
        assert len(rw) == len(video), "Reward list must have the same length as the video"
        for i, img in enumerate(video):
            try:
                ...
                if i < len(rw) - 1:
                    cv2.putText(img_bgr, f"Setp Reward: {rw[i]}", (10, 30), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                else:
                    cv2.putText(img_bgr, f"Epi Return: {rw[-1]}", (10, 30), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
def parse_config_excluded_predicates(
    ...
    if CFG.excluded_predicates:
        ...
        elif CFG.excluded_predicates == "all+goal":
...
def get_se3_hand_pose_from_state(
        state: State, obj: Object) -> math_helpers.SE3Pose:  # pragma: no cover
    """Helper for spot environments."""
    return math_helpers.SE3Pose(
        state.get(obj, "ee_x"), state.get(obj, "ee_y"), state.get(obj, "ee_z"),
        math_helpers.Quat(state.get(obj, "ee_qw"), state.get(obj, "ee_qx"),
                          state.get(obj, "ee_qy"), state.get(obj, "ee_qz")))
...
def get_important_cfg(cfg, dict):
    ...
    # conbine cfg and dict
    for k, v in dict.items():
        setattr(cfg, k, v)
    config = {}
    for k in important_keys:
        assert hasattr(cfg, k), f"Config is missing key {k}"
        config[k] = getattr(cfg, k)
    return config

def task2json(task: EnvironmentTask, base_path: str):
    task_dict = {
        "objects": {},
        "init": {},
        "goal": {}
    ...
    for obj in list(task.init):
        task_dict["objects"][obj.name] = obj.type.name
        task_dict["init"][obj.name] = {}
        for feat_name in obj.type.feature_names:
            value = task.init.get(obj, feat_name)
            if isinstance(value, np.float32):
                value = float(value)
            if isinstance(value, np.ndarray):
                value = value.tolist()
            task_dict["init"][obj.name][feat_name] = \
                value
    for goal in list(task.goal):
        if goal.predicate.name not in task_dict["goal"]:
            task_dict["goal"][goal.predicate.name] = []
        goal_objects = goal.objects
        task_dict["goal"][goal.predicate.name].append(
            [goal_object.name for goal_object in goal_objects]
        )
...
def parse_json_ivntr_info(json_dict: Dict,
    ...
    selected_pred = {}
    init_names2pred = {
        pred.name: pred for pred in init_predicates
    }
    temp_name2pred = {}
    for k, v in json_dict['pred_temp'].items():
        name = k
        types = [domain_types[t] for t in v]
        if name in init_names2pred:
            pred_template = init_names2pred[name]
        else:
            pred_template = DummyPredicate(name=name, types=types)
            temp_name2pred[name] = pred_template
        learned_ae_pred_info[pred_template] = {}
    
    for temp_name, info in json_dict['learned_ae_pred_info'].items():
        if temp_name in temp_name2pred:
            pred_template = temp_name2pred[temp_name]
        else:
            assert temp_name in init_names2pred, \
                f"Template {temp_name} not found in init predicates"
            pred_template = init_names2pred[temp_name]
        learned_ae_pred_info[pred_template] = info

    for temp_name, value in json_dict['pred'].items():
        if temp_name in init_names2pred:
            pred_temp = init_names2pred[temp_name]
            selected_pred[pred_temp] = []
        elif temp_name in temp_name2pred:
            pred_temp = temp_name2pred[temp_name]
            selected_pred[pred_temp] = []
        else:
            raise ValueError(f"Selected pred {temp_name} not found in init predicates")
        
        for v in value:
            pred_name = v['name']
            pred_types = [domain_types[t] for t in v['types']]
            pred = DummyPredicate(name=pred_name, types=pred_types)
            selected_pred[pred_temp].append(
                (pred, v['i'], v['m'])
            )
    return learned_ae_pred_info, selected_pred
        
def parse_str2nsrt(dummy_nsrt_dict: Dict,
                   dummy2real_pred: Dict[Predicate, Predicate],
                   init_predicates: set[Predicate],
                   init_options: set[Option],
                   domain_types: Dict[str, Type],
    ...
    nsrts = []
    for k, v in dummy_nsrt_dict.items():
        name = k
        parameter_types = []
        for param in v['parameters']:
            parameter_types.append(domain_types[param['type']])
        parameters = create_new_variables(
                    [t for t in parameter_types])
        option_var_types = []
        for var_type in v['option_vars']:
            option_var_types.append(domain_types[var_type['type']])
        option_vars = create_new_variables(
                    [t for t in option_var_types])
        param_name2var = {}
        for i, param in enumerate(parameters):
            assert param.name == option_vars[i].name
            assert param.type == option_vars[i].type
            param_name2var[param.name] = param

        preconditions = set()
        for pred, var_tuples in v['preconditions'].items():
            assert pred in all_predicate_str2pred, \
                f"Predicate {pred} not found"
            predicate = all_predicate_str2pred[pred]
            for var_tuple in var_tuples:
                if predicate.arity == 0:
                    preconditions.add(LiftedAtom(predicate, []))
                else:
                    var_list = []
                    for var in var_tuple:
                        var_list.append(param_name2var[var])
                    preconditions.add(LiftedAtom(predicate, var_list))

        add_effects = set()
        for pred, var_tuples in v['add_effects'].items():
            assert pred in all_predicate_str2pred, \
                f"Predicate {pred} not found"
            predicate = all_predicate_str2pred[pred]
            for var_tuple in var_tuples:
                if predicate.arity == 0:
                    add_effects.add(LiftedAtom(predicate, []))
                else:
                    var_list = []
                    for var in var_tuple:
                        var_list.append(param_name2var[var])
                    add_effects.add(LiftedAtom(predicate, var_list))

        delete_effects = set()
        for pred, var_tuples in v['delete_effects'].items():
            assert pred in all_predicate_str2pred, \
                f"Predicate {pred} not found"
            predicate = all_predicate_str2pred[pred]
            for var_tuple in var_tuples:
                if predicate.arity == 0:
                    delete_effects.add(LiftedAtom(predicate, []))
                else:
                    var_list = []
                    for var in var_tuple:
                        var_list.append(param_name2var[var])
                    delete_effects.add(LiftedAtom(predicate, var_list))
        option = all_option_str2option[v['option_name']]
        # no samplers for now
        if create_sampler:
            ...
            reg_weight_path = f"{approach_save_path}_sampler_op_{name}_reg_weight.pt"
            reg_info_path = f"{approach_save_path}_sampler_op_{name}_reg_info.pt"
            if not os.path.exists(cls_weight_path) or not os.path.exists(reg_weight_path):
                logging.warning(f"Sampler weights not found for {name}, using GT sampler")
                sampler = None
            else:
                from predicators.nsrt_learning.sampler_learning import _create_neural_sampler
                logging.info(f"Creating neural sampler for {name}")
        ...
        nsrt = NSRT(name=name, parameters=parameters, preconditions=preconditions,
                    ...
                    option_vars=option_vars, _sampler=sampler)
        nsrts.append(nsrt)
    return nsrts
