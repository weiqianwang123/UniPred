...
class Discover(object):
    def __init__(self, args):
        ...
        if self.env_name == 'satellites_simple':
            ...
            if self.num_ent == 3:
                self.next_state_offset = 5
                # This part is hard coded now, can't generalize
                self.node_name_mapping = {
                    0: 'A_i',  
                    1: 'Oc_S_0',  2: 'Oc_S_1', 3: 'Oc_S_2', 4: 'Rel_S_3', 5: 'Rel_S_4', 6: 'Rel_S_5',
                    7: 'Oc_NS_0', 8: 'Oc_NS_1', 9: 'Oc_NS_2', 10: 'Rel_NS_3', 11: 'Rel_NS_4', 12: 'Rel_NS_5'
                }
            else:
            ...
            self.pvalue_threshold = 0.01
            self.num_ent = args['env_params']['num_blocks'] + 1 # blocks + robot
            self.num_obj_pairs = self.num_ent * (self.num_ent - 1)
            self.state_dim_list = [args['env_params']['state_dim']] * self.num_ent
            ...
            # nodes (Action x1, state x20)
            if self.num_ent == 5:
                self.next_state_offset = 37
                # This part is hard coded now, can't generalize
                self.node_name_mapping = {
                    0: 'A_i'
                }
                for i in range(1, 7):
                    self.node_name_mapping[i] = 'Oc_S_' + str(i-1)
                for i in range(7, 37):
                    self.node_name_mapping[i] = 'Rel_S_' + str(i-7)
                for i in range(37, 43):
                    self.node_name_mapping[i] = 'Oc_NS_' + str(i-37)
                for i in range(43, 73):
                    self.node_name_mapping[i] = 'Rel_NS_' + str(i-43)
            else:
                raise ValueError('unknown env name')
            ...
            self.remove_list = [[] for _ in self.node_name_mapping.keys()]
            # variable type is discrete or not
            self.discrete_var = {0: False}
            for i in range(1, 7):
                self.discrete_var[i] = False
            for i in range(7, 37):
                self.discrete_var[i] = True
            for i in range(37, 43):
                self.discrete_var[i] = False
            for i in range(43, 73):
                self.discrete_var[i] = True
    ...
    def store_transition(self, data):
        ...
        state = data[0][:len(data[0])]
        action = data[1]
        next_state = data[2][:len(data[0])]
    ...
    def update_causal_graph(self):
        ...
        if self.env_name in ['satellites_simple']:
            data_dict = {}
    ...
    def save_model(self, info):
        states = {'causal_graph': self.causal_graph}
        info.update({'graph': states})
        return info

    def load_model(self, info):
        causal_graph = info['graph']
