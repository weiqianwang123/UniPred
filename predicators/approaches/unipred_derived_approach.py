...
from predicators.llm_bilevel_learning.interface import LLMEffectVectorGenerator,PDDLEffectVectorGenerator,constraints_to_vector
from predicators.predicate_search_score_functions import _register_active_derived
...
def train_val_model_in_parallel(curr_pred, ent_idx, pred_save_path, ae_vector, iteration, pred_config, queue, \
    ...
    queue.put((n,save_path_model, best_val_loss))
...
class BilevelLearningLLMApproach(NSRTLearningApproach):
    ...
    def gen_ae_vectors4pred(self, iteration: int, curr_pred: DummyPredicate, ent_idx: List[int], pred_save_path: str, \
        ...
        if len(curr_ae_vectors) == 1 or (not CFG.neupi_parallel_invention):
            ...
            if CFG.neupi_parallel_invention:
                ...
                for i in range(len(sat_vectors)):
                    model_weight_paths.append(model_paths_loss[i][1])
                    val_losses.append(model_paths_loss[i][2])
                    score = torch.load(model_paths_loss[i][1].replace("model", "guidance"))
                    scores.append(score)
                    ae_vec = torch.load(model_paths_loss[i][1].replace("model", "ae_vector"))
    ...
    def learn_neural_predicates(
        ...
        pddl_generator = PDDLEffectVectorGenerator(                  
                    target_preds   = all_target_preds,
                    sorted_options = list(self._sorted_options),
                    domain_desc    = "",
                    other_predicates = self._initial_predicates,
                    demo_prompt    = "",
                    pddl_config_path = CFG.pred_pddl_config)
        
        llm_effect_bank: torch.Tensor = pddl_generator.generate()    # <- ONE OpenAI call
        ...
        for curr_pred in list(sorted(self.learned_ae_pred_info.keys(), key=lambda p: p.arity, reverse=True)):
            ...
            if os.path.exists(CFG.neupi_load_pretrained) or pred_config["skip_train"]:
                ...
                if num_vectors_to_generate == 0:
                    logging.info(f"Skipping training for {curr_pred} since num_vectors_to_generate is 0")
                    self.learned_ae_pred_info[curr_pred]['learned'] = True
                    continue
                ...
                if sat_vec_pairs:
                    iter_ae_vectors, iter_guidance_vecs, iter_val_loss, model_weight_paths,iter_names = \
                        self.gen_ae_vectors4pred_pddl(curr_pred, ent_idx, pred_save_path, \
                                                pred_config, data,sat_vec_pairs,iteration)
                    ...
                    logging.info(f"Learning Done for pddl derived predicate {curr_pred.name}!")
                    # ------------------------------------------------------------------
            ...
            logging.info(f"*****PDDL completion for {curr_pred.name} done, now generating remainning vectors...****")     
            ...
            if sat_vec_pairs:
                for ae_vec, vloss in zip(iter_ae_vectors, iter_val_loss):
                    key = llm_generator._vec_to_key(ae_vec)
                    llm_generator._seen.add(key)
                    llm_generator.update_loss(two2one(ae_vec), float(vloss))
                    logging.info(f"Initialized LLM loss memory with PDDL-derived AE vec ({curr_pred.name}): loss={float(vloss):.4f}")
            # for pddl_name, row_vec in rows_with_names:
            #     key =  llm_generator._vec_to_key(row_vec)  # Convert to key format: ADD:[...]|DEL:[...]
            #     llm_generator._seen.add(key) 
            ...
            for i in range(num_basic_pred):
                ...
                if iteration >= 0:
                    ...
                    # Check if files exist before renaming (they might already be renamed or not saved)
                    if os.path.exists(saved_model_path):
                        ...
                        # File might already be renamed or path is incorrect, try the _good version
                        good_path = saved_model_path.replace("_model", "_model_good")
                        if os.path.exists(good_path):
                            self.learned_ae_pred_info[curr_pred]['model_weights'][i] = good_path
                        else:
                            logging.warning(f"Model file not found: {saved_model_path}")

                    saved_guidance_path = saved_model_path.replace("_model", "_guidance")
                    if os.path.exists(saved_guidance_path):
                    ...
                    if os.path.exists(saved_ae_vector_path):
                ...
                generate_derived = pred_config.get('generate_derived', True)  # default to True for backward compatibility
                if (CFG.neupi_w_negation or CFG.neupi_w_quantifiers) and generate_derived:
                    if curr_pred.arity == 1:
            ...
            for i, ae_vec_group in enumerate(ae_vec_groups):
                ...
                for m, ae_vec in enumerate(ae_vec_group):
                    ...
                    if pred_id in seen:
                        ...
                        continue
                    seen[pred_id] = name 
                    colind2pred_info[len(huge_ae_matrix_invented)-1] = (neual_pred, local_ent_idx, False, (pred, i, m,list(quantified_type_idx)))
    ...
    def _get_invented_predicates_from_info(self, info: Dict, from_json=False) -> \
        ...
        for pred_temp in info.keys():
            if pred_temp in self._initial_predicates or pred_temp.name == "goalAchieved":
    ...
    def _select_predicates_by_score_search(
        self, 
        huge_ae_matrixe: np.ndarray,
        # all_predicates_info: {col_index: (pred, ent_idx, is_provided, (basic_pred, group_idx, var_idx, extra))}
        all_predicates_info: Dict[int, Tuple[Predicate, List[int], bool, Tuple[Predicate, int, int]]],
        score_function: _OperatorBeliefScoreFunction
    ) -> Tuple[Set[Predicate], np.ndarray, List[Predicate], List[List[int]]]:
        """Greedy search with derived support.

        变更要点：
        - basic_to_derived / derived_to_basic 都改为以 (basic_pred, group_idx) 作为 key，
        从而区分 neural_b_table_toy_0 与 neural_b_table_toy_1 两个基础组的派生。
        - imagined 只选 basic；imagined 回退时清理 pending 的其派生。
        - evaluate 时将 basic_only_candidate_predicates 仅传当前已选效应里的 basic 集合。
        """
        import copy, random, torch, logging

        # ---------------- 预处理：索引与 var_idx 映射 ----------------
        provided_effect_predicates: List[Predicate] = []
        provided_prec_predicates: List[Predicate] = []
        basic_matrix: List[torch.Tensor] = []
        basic_pred_ent_idx: List[List[int]] = []
        possible_cols: List[int] = []
        possible_predicates: List[Predicate] = []
        possible_pred_entidx: List[List[int]] = []

        # key = (basic_pred, group_idx)
        basic_to_derived: Dict[Tuple[Predicate, int], List[int]] = {}
        derived_to_basic: Dict[int, Tuple[Predicate, int]] = {}
        basic_pred_cols_by_key: Dict[Tuple[Predicate, int], int] = {}

        # 建立 pred -> var_idx（默认 0 当 basic）
        pred2varidx: Dict[Predicate, int] = {}

        for colind, pred_info in all_predicates_info.items():
            if len(pred_info) == 3:
                pred, ent_idx, is_provided = pred_info
                meta_info = None
            else:
                pred, ent_idx, is_provided, meta_info = pred_info

            # 记录 var_idx
            if meta_info is not None and isinstance(meta_info, tuple):
                # meta_info = (basic_pred, group_idx, var_idx, extra...)
                _, _, var_idx, *_ = meta_info
                pred2varidx[pred] = var_idx
            else:
                pred2varidx.setdefault(pred, 0)

            if is_provided:
                logging.info(f"Provided Predicate: {pred}")
                provided_effect_predicates.append(pred)
                basic_matrix.append(huge_ae_matrixe[:, colind])
                basic_pred_ent_idx.append(ent_idx)
            else:
                logging.info(f"Possible Predicate: {pred}")
                possible_cols.append(colind)
                possible_predicates.append(pred)
                possible_pred_entidx.append(ent_idx)

                if meta_info is not None and isinstance(meta_info, tuple):
                    basic_pred, group_idx, var_idx, *_ = meta_info
                    key = (basic_pred, group_idx)
                    if getattr(basic_pred, "arity", None) == 2:
                        if var_idx > 0:
                            # derived
                            basic_to_derived.setdefault(key, []).append(colind)
                            derived_to_basic[colind] = key
                        else:
                            # basic
                            basic_pred_cols_by_key[key] = colind
                            basic_to_derived.setdefault(key, [])

        # 供前置使用的 provided 初始谓词（若有）
        for pred in list(self._initial_predicates):
            if pred in provided_effect_predicates:
                continue
            provided_prec_predicates.append(pred)

        # 一个小工具：从“当前效应谓词列表”里挑出 basic，用于学习算子
        def _learning_basics(try_effect_preds: List[Predicate]) -> List[Predicate]:
            return [p for p in try_effect_preds if pred2varidx.get(p, 0) == 0]

        # ---------------- 搜索状态 ----------------
        searched_predicate_comp: List[Set[Predicate]] = []
        final_predicates: List[Predicate] = copy.deepcopy(provided_effect_predicates)
        level = 0
        go_on = True
        imagined_search = False
        imagined_pred: List[Predicate] = []
        last_matrix = (torch.stack(basic_matrix, dim=1) 
                    if len(basic_matrix) > 0 
                    else torch.empty((huge_ae_matrixe.shape[0], 0)))
        last_ent_idx: List[List[int]] = copy.deepcopy(basic_pred_ent_idx)
        search_record: Dict[int, Tuple[Predicate, float]] = {}
        best_score = float("inf")

        current_imagined = None  # {'pred': Predicate, 'col': int, 'owned_derived': Set[int]}
        selected_basic_keys: Set[Tuple[Predicate, int]] = set()
        pending_derived_cols: List[int] = []

        logging.info(f"Found {len(basic_to_derived)} basic-(group)-to-derived sets")
        for (basic_pred, group_idx), derived_cols in basic_to_derived.items():
            logging.info(f"Basic {getattr(basic_pred,'name',str(basic_pred))}[group {group_idx}] "
                        f"has {len(derived_cols)} derived variants")
        ...
        if CFG.neupi_bug:
            for idx, p_pred in enumerate(possible_predicates):
                ...
                try_effect_preds = list(final_predicates)  # 效应段
                learning_basics = _learning_basics(try_effect_preds)
                score = score_function.evaluate(last_matrix,
                                                try_effect_preds + provided_prec_predicates,
                                                copy.deepcopy(last_ent_idx),
                                                CFG.strips_learner,
                                                all_predicates_info=all_predicates_info,
                                                basic_only_candidate_predicates=learning_basics)
                logging.info(f"Predicate set {try_effect_preds} (+preconds) score {score}.")
        ...
        while go_on:
            ...
            # ① 先评估 pending 派生
            if pending_derived_cols:
                logging.info(f"Evaluating {len(pending_derived_cols)} pending derived predicates...")
                for derived_col in list(pending_derived_cols):
                    if derived_col not in possible_cols:
                        pending_derived_cols.remove(derived_col)
                        continue
                    derived_idx = possible_cols.index(derived_col)
                    derived_pred = possible_predicates[derived_idx]
                    if derived_pred in final_predicates:
                        pending_derived_cols.remove(derived_col)
                        continue

                    try_matrix = torch.cat([last_matrix, huge_ae_matrixe[:, derived_col:derived_col+1]], dim=1)
                    try_pred_ent_idx = copy.deepcopy(last_ent_idx)
                    try_pred_ent_idx.append(possible_pred_entidx[derived_idx])
                    ...
                    (self.check_matrix_consistency(last_matrix, last_ent_idx)):
                        logging.info(f"Derived predicate matrix is not consistent, skip...")
                        pending_derived_cols.remove(derived_col)
                        continue

                    try_effect_preds = list(final_predicates) + [derived_pred]
                    learning_basics = _learning_basics(try_effect_preds)

                    searched_predicate_comp.append(set(try_effect_preds))
                    score = score_function.evaluate(try_matrix,
                                                    try_effect_preds + provided_prec_predicates,
                                                    try_pred_ent_idx,
                                                    CFG.strips_learner,
                                                    all_predicates_info=all_predicates_info,
                                                    basic_only_candidate_predicates=learning_basics)
                    logging.info(f"Derived set {getattr(derived_pred, 'name', str(derived_pred))} score {score}.")
                ...
                if add_idx != -1:
                    added_pred = possible_predicates[add_idx]
                    logging.info(f"Adding derived predicate {added_pred}...")
                    col_idx_add = possible_cols[add_idx]
                    final_predicates.append(added_pred)
                    ...
                    search_record[level] = (added_pred, best_score)
                    if col_idx_add in pending_derived_cols:
                        pending_derived_cols.remove(col_idx_add)
                    ...
                    if best_score < CFG.domain_aaai_thresh:
                        ...
                        if imagined_search:
                            imagined_search = False
                            imagined_pred = []
                            current_imagined = None
                    continue

            # ② 常规候选（basic 或 pending 中的派生）
            for idx, p_pred in enumerate(possible_predicates):
                if p_pred in final_predicates:
                    continue
                col_idx = possible_cols[idx]
                # 非 pending 的派生不评估
                if col_idx in derived_to_basic and col_idx not in pending_derived_cols:
                    continue
                ...
                try_effect_preds = list(final_predicates) + [p_pred]
                learning_basics = _learning_basics(try_effect_preds)
                ...
                score = score_function.evaluate(try_matrix,
                                                ...
                                                all_predicates_info=all_predicates_info,
                                                basic_only_candidate_predicates=learning_basics)
                logging.info(f"Predicate set {try_effect_preds} score {score}.")
                if score < best_score:
                    best_score = score
                    add_idx = idx
            ...
            if add_idx != -1:
                added_pred = possible_predicates[add_idx]
                logging.info(f"New best score at level {level}: {best_score} -> add {added_pred}")
                col_idx_add = possible_cols[add_idx]
                final_predicates.append(added_pred)
                last_matrix = torch.cat([last_matrix, huge_ae_matrixe[:, col_idx_add:col_idx_add+1]], dim=1)
                last_ent_idx.append(possible_pred_entidx[add_idx])
                ...
                # 若是 basic，把它的派生推进 pending（按 (basic_pred, group_idx)）
                meta_info = all_predicates_info[col_idx_add][3] if len(all_predicates_info[col_idx_add]) >= 4 else None
                if meta_info is not None and isinstance(meta_info, tuple):
                    basic_pred, group_idx, var_idx, *_ = meta_info
                    key = (basic_pred, group_idx)
                    if var_idx == 0 and key in basic_to_derived:
                        for dcol in basic_to_derived[key]:
                            if dcol not in pending_derived_cols:
                                pending_derived_cols.append(dcol)
                        selected_basic_keys.add(key)
            ...
            # ③ 卡住：imagined（只选 basic）
            elif add_idx == -1 and best_score > CFG.domain_aaai_thresh:
                logging.info(f"Hill-Climbing stuck at level {level}, try imagined...")

                # 若上一层正处于 imagined，先回退并清理它的派生
                if imagined_search:
                    last_imag = imagined_pred[-1]
                    final_predicates.remove(last_imag)
                    last_matrix = last_matrix[:, :-1]
                    last_ent_idx = last_ent_idx[:-1]
                    level -= 1
                    if current_imagined is not None:
                        owned = set(current_imagined.get('owned_derived', []))
                        if owned:
                            pending_derived_cols = [c for c in pending_derived_cols if c not in owned]
                    current_imagined = None

                # 随机挑一个 basic imagined
                wrong_counter = 0
                chosen_idx = None
                while wrong_counter <= 100:
                    rnd_idx = random.randint(0, len(possible_predicates)-1)
                    rnd_pred = possible_predicates[rnd_idx]
                    if rnd_pred in final_predicates or rnd_pred in imagined_pred:
                        wrong_counter += 1
                        continue
                    # 只允许 basic
                    if pred2varidx.get(rnd_pred, 0) != 0:
                        wrong_counter += 1
                        continue
                    chosen_idx = rnd_idx
                    imagined_pred.append(rnd_pred)
                    imagined_search = True
                    rnd_col = possible_cols[rnd_idx]

                    # 取它所属的 (basic_pred, group_idx)，计算它“拥有的派生”
                    meta_info = all_predicates_info[rnd_col][3] if len(all_predicates_info[rnd_col]) >= 4 else None
                    if meta_info is not None and isinstance(meta_info, tuple):
                        base_pred, group_idx, _, *_ = meta_info
                        key = (base_pred, group_idx)
                    else:
                        key = None
                    owned_derived = set(basic_to_derived.get(key, [])) if key is not None else set()
                    current_imagined = {'pred': rnd_pred, 'col': rnd_col, 'owned_derived': owned_derived}
                    break

                if chosen_idx is None:
                    logging.info("cannot find any new imagined basic predicate, stop searching.")
                    return set(final_predicates), last_matrix, final_predicates, last_ent_idx

                # 加入 imagined basic，并把它的派生入 pending
                logging.info(f"Adding Imagined (BASIC) Predicate {possible_predicates[chosen_idx]}...")
                col_idx_add = possible_cols[chosen_idx]
                final_predicates.append(possible_predicates[chosen_idx])
                last_matrix = torch.cat([last_matrix, huge_ae_matrixe[:, col_idx_add:col_idx_add+1]], dim=1)
                last_ent_idx.append(possible_pred_entidx[chosen_idx])
                if current_imagined and current_imagined['owned_derived']:
                    for dcol in current_imagined['owned_derived']:
                        if dcol not in pending_derived_cols:
                            pending_derived_cols.append(dcol)
                level += 1
    ...
    def _solve_real(self, task: Task, timeout: int):
        self._num_calls += 1
        ...
        if CFG.sesame_task_planner == "astar":
            colind2info = self._build_colind_info()
            _register_active_derived(set(preds), colind2info)
    ...
    def _build_colind_info(self) -> Tuple[torch.Tensor, Dict[int, Tuple[Any, List[int], bool, Tuple[Any, int, int, List[int]]]]]:
        """
        构造（可能包含重复/等价项的）发明谓词矩阵与列索引信息映射。
        与 _get_invented_pruned_predicates 的主要区别：
        - 不需要 traj
        - 不做等价性检查与剪枝（不计算 _get_predicate_identifier，不维护 seen）
        返回:
        huge_ae_matrix_invented: torch.Tensor [effect_dim, num_invented]
        colind2pred_info: Dict[col_index] -> (predicate_or_neural_pred, ent_idx, is_provided, (orig_pred, group_i, m, quantified_type_idx_list))
        """
        colind2pred_info: Dict[int, Tuple[Any, List[int], bool, Tuple[Any, int, int, List[int]]]] = {}
        huge_ae_matrix_invented: List[torch.Tensor] = []

        # 仅当需要复现实验时启用（与你的原代码一致）
        gt_dict = {
            'neural_u_p1': {0: [0]},
            'neural_u_p2': {0: [0]},
            'neural_b_p3': {0: [0], 1: [0, 1]},
            'neural_b_p4': {0: [0], 1: [0], 2: [0]},
            'neural_b_p5': {},
            'neural_b_p6': {}
        }
        ...
        for pred in self.learned_ae_pred_info.keys():
            ...
            for item in self.pred_config:
                if item['name'] == pred.name:
                    pred_config = copy.deepcopy(item)
                    break
            assert pred_config is not None, f"Predicate config for {pred.name} not found."
            ...
            # 遍历每个 AE 向量分组
            for i, ae_vec_group in enumerate(ae_vec_groups):
                if CFG.neupi_bug and (i not in gt_idex_groups):
                    continue

                # 同一 group 共享同一个模型
                pred_model = setup_neupi_mlp_net(
                    example_dataset,
                    pred.arity,
                    pred_config['architecture'],
                    self._node_feature_to_index,
                ...
                for m, ae_vec in enumerate(ae_vec_group):
                    ...
                    # 解析量词/取反/基本谓词名
                    name = ""
                    quantifier_str, neg_str, basic_pred_str = self.learned_ae_pred_info[pred]['quantifiers'][i][m]
                    original_types = pred.types
                    original_type_names = [t.name for t in original_types]

                    quantified_type_idx, local_ent_idx = parse_basic_pred_name(basic_pred_str, original_type_names)
                    assert (np.array(local_ent_idx) == np.array(self.learned_ae_pred_info[pred]['ent_idx'][i][m])).all()
                    ...
                    # 生成分类器
                    specific_function = self.generate_classifier(
                        pred,
                        pred_model,
                        pred_config['gumbel_temp'],
                        pred_config['decision_b'],
                    ...
                    neural_pred = NeuralPredicate(name=name, types=types, _classifier=specific_function)

                    # 不做等价性检查/剪枝，直接记录
                    colind2pred_info[len(huge_ae_matrix_invented) - 1] = (
                        neural_pred,                # 构造后的谓词
                        local_ent_idx,              # 实体索引（局部）
                        False,                      # 非 provided
                        (pred, i, m, list(quantified_type_idx))  # 追踪来源
                    )

        if len(huge_ae_matrix_invented) == 0:
            raise RuntimeError("No invented AE vectors found to stack.")

        huge_ae_matrix_invented = torch.stack(huge_ae_matrix_invented, dim=1)
        return colind2pred_info
    ...
    def gen_ae_vectors4pred_pddl(
        ...
        if use_parallel:
            ...
            while not queue.empty():
                idx,  model_path, val_loss = queue.get()
