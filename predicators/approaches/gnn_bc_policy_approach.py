...
from predicators.structs import Action, Array, MDPDataset, DummyOption, \
    GroundAtom, Object, ParameterizedOption, Predicate, State, Task, Type, \
    _Option


class GNNBCPolicyApproach(GNNApproach):
    ...
    def _generate_data_from_dataset(
        ...
        data = []
        # GNN BC only takes positive trajs
        pos_traj = []
    ...
    def _global_criterion(self, output: torch.Tensor,
        ...
        if return_dict:
            loss_dict = {
                "cls": onehot_loss.item(),
                "reg": params_loss.item(),
            }
            return loss_dict, onehot_loss + params_loss
        ...
        while time.perf_counter() - start_time < timeout:
            ...
            while total_num_act < CFG.horizon:
                ...
                if time.perf_counter() - start_time > timeout:
                    break
        logging.info(f"Shooting: {all_num_act} actions with {tries} tries in {time.perf_counter() - start_time} seconds")
