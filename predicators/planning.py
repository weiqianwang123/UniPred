...
class _NodeMDP:
    """A node for the search over skeletons."""
    atoms: Set[GroundAtom]
    skeleton_op: List[_GroundNSRT]
    skeleton_nd: List[int]
    atoms_sequence: List[Set[GroundAtom]]  # expected state sequence
    parent: Optional[_Node]
    cumulative_cost: float
    ...
    while True:
        ...
        try:
            ...
            for skeleton, atoms_sequence in gen:
                if CFG.env == "spot_pickplace_stair" and \
                    (not CFG.in_domain_test):
                    # a hack to make real spot test safe (always pick the cloese stair)
                    if skeleton[0].objects[1].name != 'stair0':
                        continue
...
def _sesame_plan_for_mdp(
    ...
    allow_noops: bool = False,
    use_visited_state_set: bool = False
) -> Tuple[List[_Option], List[_GroundNSRT], Metrics, Dict[int, Tuple[_Node, float, float]]]:
    """The default version of SeSamE, which runs A* to produce skeletons."""
    init_atoms = utils.abstract(task.init, predicates)
    objects = list(task.init)
    start_time = time.perf_counter()
    ...
    while True:
        ...
        heuristic = utils.create_task_planning_heuristic(
            ...
            predicates, objects)
        new_seed = seed + int(metrics["num_failures_discovered"])
        pos_skeletons, all_nodes, all_par_nodes = _skeleton_generator_with_graph(
            task, reachable_nsrts, init_atoms, heuristic, new_seed,
            timeout - (time.perf_counter() - start_time), metrics,
            max_skeletons_optimized, use_visited_state_set)
        # get all the atoms_sequences in positive skeletons
        # they are used in reward definition
        pos_atoms = []
        for _, _, _, atoms_sequence in pos_skeletons:
            for seq in atoms_sequence:
                if seq not in pos_atoms:
                    pos_atoms.append(seq)
        
        refinement_start_time = time.perf_counter()
        # collecting low-level mdp transitions
        pos_end_nodes_mdp = {}
        neg_end_nodes_mdp = {}
        mdp_id = 0
        worst_succ_return = 0
        pos_end_nodes = []
        # first get the low level states for the skeletons (successful high-level plans)
        logging.info(f"Collecting low-level MDP transitions for successful high-level {len(pos_skeletons)} plans")
        while len(pos_end_nodes_mdp.keys()) < CFG.max_num_pos_mdp_traj:
            for node_id, skeleton_op, skeleton_nd, atoms_sequence in pos_skeletons:
                new_seed += mdp_id
                atoms_seq = atoms_sequence
                local_metric = metrics.copy()
                plan, rw, suc, has_wrong_hl = run_low_level_search_reward(
                    task, option_model, skeleton_op, skeleton_nd, atoms_seq, node_id, all_par_nodes,
                    new_seed, CFG.mdp_ll_search_timeout, local_metric,
                    max_horizon, correct_hl=True, pos_atoms=pos_atoms)
                if suc:
                    assert len(plan) == len(skeleton_op)
                    if len(pos_end_nodes_mdp.keys()) >= CFG.max_num_pos_mdp_traj:
                        if node_id not in pos_end_nodes:
                            pos_end_nodes.append(node_id)
                        worst_succ_return = min(worst_succ_return, sum(rw))
                        continue
                    else:
                        pos_end_nodes_mdp[mdp_id] = (plan, skeleton_op, rw, local_metric)
                        mdp_id += 1
                        assert not has_wrong_hl, "Positive skeleton should not have a wrong high-level plan"
                        # Success! It's a complete plan.
                        logging.info(
                            f"{len(pos_end_nodes_mdp.keys())}/{CFG.max_num_pos_mdp_traj} Planning succeeded! Plan of length "
                            f"{len(plan)} after "
                            f"skeletons with {int(local_metric['num_samples'])}"
                            f" samples, discovering "
                            f"{int(local_metric['num_failures_discovered'])} failures"
                            f" and {sum(rw)} return")
                        worst_succ_return = min(worst_succ_return, sum(rw))
                        local_metric["plan_length"] = len(plan)
                        local_metric["refinement_time"] = (time.perf_counter() -
                                                    refinement_start_time)
                        local_metric["end_node_id"] = node_id
                        local_metric["return"] = sum(rw)
                # else:
                #     logging.info(
                #         f"{len(neg_end_nodes_mdp.keys())}/{CFG.max_num_neg_mdp_traj} Planning Failed! Plan of length "
                #         f"{len(plan)} after "
                #         f"{int(metrics['num_samples'])}"
                #         f" samples, discovering "
                #         f"{int(metrics['num_failures_discovered'])} failures"
                #         f" and {sum(rw)} return")
                #     metrics["plan_length"] = len(plan)
                #     metrics["refinement_time"] = (time.perf_counter() -
                #                                 refinement_start_time)
                #     metrics["end_node_id"] = node_id
                #     metrics["return"] = sum(rw)
                #     neg_end_nodes_mdp[mdp_id] = (plan, skeleton_op, rw, metrics)
                #     mdp_id += 1
    
        # Then get the low level states for the skeletons (successful high-level plans)
        logging.info(f"Collecting low-level MDP transitions for failed high-level plans")
        # Get all the end nodes for the negative skeletons
        neg_end_nodes = []
        for node_id in all_nodes.keys():
            if node_id in pos_end_nodes:
                continue
            if node_id in all_par_nodes.keys():
                continue
            neg_end_nodes.append(node_id)
        while len(neg_end_nodes_mdp.keys()) < CFG.max_num_neg_mdp_traj:
            # sample a negative skeleton
            new_seed += mdp_id
            node_id = np.random.choice(neg_end_nodes)
            skeleton_op, skeleton_nd, atoms_sequence = all_nodes[node_id].skeleton_op \
                , all_nodes[node_id].skeleton_nd, all_nodes[node_id].atoms_sequence
            atoms_seq = atoms_sequence
            if task.goal.issubset(atoms_seq[-1]):
                logging.info("Negative skeleton should not have a successful plan")
                continue
            local_metric = metrics.copy()
            plan, rw, suc, has_wrong_hl = run_low_level_search_reward(
                task, option_model, skeleton_op, skeleton_nd, atoms_seq, node_id, all_par_nodes,
                new_seed, CFG.mdp_ll_search_timeout, local_metric,
                max_horizon, correct_hl=False, pos_atoms=pos_atoms)
            assert not suc, "Negative skeleton should not have a successful plan, skip"
            if sum(rw) >= worst_succ_return:
                logging.info("This plan is better than the worst successful plan, skip")
                continue
            if not has_wrong_hl:
                logging.info("This plan is not a wrong high-level plan, skip")
                continue
            else:
                # Save this bad MDP.
                logging.info(
                    f"{len(neg_end_nodes_mdp.keys())}/{CFG.max_num_neg_mdp_traj} Planning succeeded! Plan of length "
                    f"{len(plan)} after "
                    f"skeletons with {int(local_metric['num_samples'])}"
                    f" samples, discovering "
                    f"{int(local_metric['num_failures_discovered'])} failures"
                ...
                local_metric["end_node_id"] = node_id
                local_metric["return"] = sum(rw)
                neg_end_nodes_mdp[mdp_id] = (plan, skeleton_op, rw, local_metric)
                mdp_id += 1

        return pos_end_nodes_mdp, neg_end_nodes_mdp
...
def _skeleton_generator_with_graph(
    task: Task,
    ground_nsrts: List[_GroundNSRT],
    init_atoms: Set[GroundAtom],
    heuristic: _TaskPlanningHeuristic,
    ...
    max_skeletons_optimized: int,
    use_visited_state_set: bool = False
) -> Tuple[List[Tuple[List[_GroundNSRT], List[Set[GroundAtom]]]], 
           Dict[int, Tuple[_Node, Optional[int], float, float]]]:
    """A* search over skeletons (sequences of ground NSRTs).
    Iterates over pairs of (skeleton, atoms sequence).
    Returns the graph of all nodes created, their heuristics, and cumulative costs, 
    this will be used in RL.
    ...
    start_time = time.perf_counter()
    current_objects = set(task.init)
    queue: List[Tuple[float, float, _NodeMDP]] = []
    root_node = _NodeMDP(atoms=init_atoms,
                      skeleton_op=[],
                      skeleton_nd=[],
                      atoms_sequence=[init_atoms],
                      parent=None,
    ...
    hq.heappush(queue,
                (heuristic(root_node.atoms), rng_prio.uniform(), root_node))
    visited_skeletons: Set[Tuple[_GroundNSRT, ...]] = set()
    visited_skeletons.add(tuple(root_node.skeleton_op))
    if use_visited_state_set:
        visited_atom_sets = set()

    # All nodes will now include parent node ID and transition reward for each child
    all_par_nodes = {id(root_node): {}}
    skeletons = [] # positive skeletons that are optimal
    all_nodes = {id(root_node): root_node}
    optimal_length = np.inf

    while queue and (time.perf_counter() - start_time < timeout):
        if int(metrics["num_skeletons_optimized"]) == max_skeletons_optimized:
            return skeletons, all_nodes, all_par_nodes
        ...
        if task.goal.issubset(node.atoms):
            if len(node.skeleton_op) <= optimal_length:
                optimal_length = len(node.skeleton_op)
                metrics["num_skeletons_optimized"] += 1
                # the node id should be unique
                skeletons.append((id(node), node.skeleton_op, node.skeleton_nd, node.atoms_sequence))
                assert id(node) in all_nodes.keys(), "Goal node not found in all_nodes!"
        else:
            metrics["num_nodes_expanded"] += 1

            for nsrt in utils.get_applicable_operators(ground_nsrts, node.atoms):
                child_atoms = utils.apply_operator(nsrt, set(node.atoms))
                if use_visited_state_set:
                    frozen_atoms = frozenset(child_atoms)
                    if frozen_atoms in visited_atom_sets:
                        continue
                child_skeleton_op = node.skeleton_op + [nsrt]
                child_skeleton_nd = node.skeleton_nd + [id(node)]
                child_skeleton_tup_op = tuple(child_skeleton_op)
                if child_skeleton_tup_op in visited_skeletons:  # pragma: no cover
                    continue
                visited_skeletons.add(child_skeleton_tup_op)
                child_cost = node.cumulative_cost + 1.0
                child_node = _NodeMDP(atoms=child_atoms,
                                   skeleton_op=child_skeleton_op,
                                   skeleton_nd=child_skeleton_nd,
                ...
                # Calculate transition reward based on heuristic difference and step cost
                transition_reward = heuristic(node.atoms) - heuristic(child_node.atoms) - 1.0
                all_nodes[id(child_node)] = child_node
                if id(node) in all_par_nodes:
                    all_par_nodes[id(node)][id(child_node)] = transition_reward
                else:
                    all_par_nodes[id(node)] = {
                        id(child_node): transition_reward
                    }
    ...
    if not queue:
        assert len(skeletons) > 0, "No skeletons found even after exhaustive search!"
        logging.info(f"Planning exhausted with {len(skeletons)} skeletons found!")
        return skeletons, all_nodes, all_par_nodes
    else:
        assert time.perf_counter() - start_time >= timeout
        assert len(skeletons) > 0, "No skeletons found even after timed out!"
        logging.info(f"Planning timed out with {len(skeletons)} skeletons found!")
        return skeletons, all_nodes, all_par_nodes


def run_low_level_search_reward(
    task: Task,
    option_model: _OptionModelBase,
    skeleton_op: List[_GroundNSRT],
    skeleton_nd: List[int],
    ...
    end_node_id: int,
    all_par_nodes: Dict[int, Dict[int, float]],
    ll_seed: int,
    timeout: float,
    metrics: Metrics,
    max_horizon: int,
    correct_hl: bool = False,
    pos_atoms: List[Set[GroundAtom]] = [],
    refinement_time: Optional[List[float]] = None
) -> Tuple[List[_Option], bool]:
    """Backtracking search over continuous values.

    Returns a sequence of options, a sequence of reward, and a boolean. If the boolean is True,
    ...
    """
    start_time = time.perf_counter()
    rng_sampler = np.random.default_rng(ll_seed)
    assert CFG.sesame_propagate_failures in \
        {"after_exhaust", "immediately", "never"}
    # santity checks
    if correct_hl:
        assert end_node_id not in all_par_nodes.keys(), \
            "Success high-level plan end node cannot be a parent node!"
    else:
        assert len(pos_atoms) > 0, "Positive atoms must be provided for negative MDP trajectory!"
    assert skeleton_nd[-1] in all_par_nodes.keys(), \
        "Last node in skeleton_nd must be a parent node!"
    # initial rewards from task planning
    # initial_rewards[i] denotes the reward of transitioning from node i to node i + 1, 
    # the last node is the end node
    initial_rewards = [0.0 for _ in skeleton_op]
    for i in range(len(skeleton_nd) - 1):
        assert skeleton_nd[i] in all_par_nodes.keys(), \
            "All nodes in skeleton_nd must be parent nodes!"
        assert skeleton_nd[i + 1] in all_par_nodes[skeleton_nd[i]].keys(), \
            "All nodes in skeleton_nd must be children of the previous node!"
        initial_rewards[i] = all_par_nodes[skeleton_nd[i]][skeleton_nd[i + 1]]
    assert end_node_id in all_par_nodes[skeleton_nd[-1]].keys(), \
        "End node must be a child of the last node in skeleton_nd!"
    initial_rewards[-1] = all_par_nodes[skeleton_nd[-1]][end_node_id]
    
    cur_idx = 0
    has_wrong_hl = False
    num_tries = [0 for _ in skeleton_op]
    ...
    max_tries = [
        ...
        if nsrt.option.params_space.shape[0] > 0 else 1 for nsrt in skeleton_op
    ]
    plan: List[_Option] = [DummyOption for _ in skeleton_op]
    ...
    traj: List[State] = [task.init] + [DefaultState for _ in skeleton_op]
    ...
    discovered_failures: List[Optional[_DiscoveredFailure]] = [
        None for _ in skeleton_op
    ]
    plan_found = False
    while cur_idx < len(skeleton_op):
        if time.perf_counter() - start_time > timeout:
            initial_rewards = initial_rewards[:len(longest_failed_refinement)]
            return longest_failed_refinement, initial_rewards, False, has_wrong_hl
        ...
        if option.initiable(state):
            try:
                ...
                # Check if the option was effectively a noop.
                    ...
                    if not correct_hl:
                        # check if the next step belongs to the positive trajectory
                        if not (frozenset(expected_atoms) in map(frozenset, pos_atoms)):
                            # this atom does not belong to the positive trajectory
                            initial_rewards[cur_idx - 1] += CFG.mdp_bad_hl_step
                            has_wrong_hl = True
        ...
        if plan_found:
            assert len(plan) == len(initial_rewards)
            if correct_hl:
                assert task.goal_holds(traj[-1]), "Good HL plan Goal not satisfied!"
                return plan, initial_rewards, True, has_wrong_hl  # success!
            else:
                assert not task.goal_holds(traj[-1]), "Bad HL plan Goal satisfied!"
                initial_rewards[-1] += CFG.mdp_fail_last_step
                return plan, initial_rewards, False, has_wrong_hl  # Fail!
        if not can_continue_on:  # we got stuck, time to resample / backtrack!
            # Update the longest_failed_refinement found so far.
            # First, decrease the reward (informed by low-level search)
            initial_rewards[cur_idx-1] += CFG.mdp_ll_step_fail_rw
            ...
            while num_tries[cur_idx] == max_tries[cur_idx]:
                ...
                if cur_idx < 0:
                    ...
                    # cut reward list, reduce reward
                    initial_rewards = initial_rewards[:len(longest_failed_refinement)]
                    for i in range(len(longest_failed_refinement)):
                        if correct_hl:
                            initial_rewards[i] -= CFG.mdp_good_hl_step_fail_rw
                        else:
                            initial_rewards[i] -= CFG.mdp_bad_hl_step_fail_rw
                    return longest_failed_refinement, initial_rewards, False, has_wrong_hl
    # Should only get here if the skeleton was empty.
    assert not skeleton_op
    return [], [], True, has_wrong_hl
...
def run_low_level_search(
    ...
    plan_found = False
    # env = ViewPlanHardEnv()
    # env = BlocksEngravePcdEnv()
    # env = PickPlaceEnv()
    # env.render_state_plt(task.init, task, save_path="0.png")
    while cur_idx < len(skeleton):
        ...
        if option.initiable(state):
            try:
                ...
                # env = BlocksEngravePcdEnv()
                # env = PickPlaceEnv()
                # env.render_state_plt(next_state, task, save_path=f"{cur_idx}.png")
                discovered_failures[cur_idx - 1] = None
                num_actions_per_option[cur_idx - 1] = num_actions
                traj[cur_idx] = next_state
                # Check if objects that were outside the scope had a change
...
def analyse_low_level_search(
    ...
    # Optimization: if the params_space for the NSRT option is empty, only
    # sample it once, because all samples are just empty (so equivalent).
    # always sample 100 times for analysis
    max_tries = [
        CFG.sesame_max_samples_per_step
        if nsrt.option.params_space.shape[0] > 0 else 1 for nsrt in skeleton
    ]
    ...
    while cur_idx < len(skeleton):
        ...
        if option.initiable(state):
            try:
                ...
                # Check if the option was effectively a noop.
                    ...
                    # utils.abstract(traj[cur_idx], predicates).
                    if all(a.holds(traj[cur_idx]) for a in expected_atoms):
                        attempts[cur_idx - 1].append((option, next_state.copy(), 1))
                        if num_tries[cur_idx - 1] < max_tries[cur_idx - 1] // 2:
                            # force it to generate data until max tries
                            can_continue_on = False
                        else:
                            can_continue_on = True
                        if cur_idx == len(skeleton):
                        ...
                        attempts[cur_idx - 1].append((option, next_state.copy(), 0))
                        can_continue_on = False
                else:
                    # If we're not checking expected_atoms, we need to
                    # explicitly check the goal on the final timestep.
        ...
        if refinement_time is not None:
            ...
            refinement_time[cur_idx - 1] += try_end_time - try_start_time
        if plan_found:
            return plan, attempts, True  # success!
        if not can_continue_on:  # we got stuck, time to resample / backtrack!
            # Update the longest_failed_refinement found so far.
            if cur_idx > len(longest_failed_refinement):
                longest_failed_refinement = list(plan[:cur_idx])
            ...
            while num_tries[cur_idx] == max_tries[cur_idx]:
                ...
                if cur_idx < 0:
                    ...
                    return longest_failed_refinement, attempts, False
    # Should only get here if the skeleton was empty.
    assert not skeleton
...
def analyse_with_fast_downward(
    task: Task, option_model: _OptionModelBase, nsrts: Set[NSRT],
    predicates: Set[Predicate], types: Set[Type], timeout: float, seed: int,
    max_horizon: int, optimal: bool
) -> Tuple[List[_Option], List[_GroundNSRT], Metrics]:  # pragma: no cover
    ...
    while True:
        ...
        try:
            necessary_atoms_seq = utils.compute_necessary_atoms_seq(
                skeleton, atoms_sequence, task.goal)
            refinement_start_time = time.perf_counter()
            plan, inter, suc = analyse_low_level_search(task, option_model, skeleton,
                                             necessary_atoms_seq, seed,
                                             low_level_timeout, metrics,
                                             max_horizon)
            if not suc:
