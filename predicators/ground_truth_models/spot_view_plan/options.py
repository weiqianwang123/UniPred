...
from predicators.spot_utils.skills.spot_navigation import \
    ...
    navigate_to_absolute_pose, navigate_to_relative_pose_collision_free
...
def _calibrate(robot: Robot) -> None:
    # Calibrate. Use Close Gripper and Open Gripper as indicators.
    close_gripper(robot)
    open_gripper(robot)

def _measure(robot: Robot) -> None:
...
def _calibrate_policy(name: str, state: State, memory: Dict, objects: Sequence[Object],
                        params: Array) -> Action:
...
def _measure_policy(name: str, state: State, memory: Dict, objects: Sequence[Object],
                        params: Array) -> Action:
    del state, memory, params  # not used
    robot, localizer, _ = get_robot()
    fn = _measure
...
def _move_away_from_object_policy(state: State, memory: Dict,
                                    objects: Sequence[Object],
                                    params: Array) -> Action:
    name = "MoveAwayFromObject"
...
def _move_away_from_object_stair_policy(state: State, memory: Dict,
                                    objects: Sequence[Object],
                                    params: Array) -> Action:
    name = "MoveAwayFromObject"
    distance_param_idx = 0
...
def _calibrate_obj_policy(state: State, memory: Dict,
                        ...
                        params: Array) -> Action:
    name = "Calibrate"
    return _calibrate_policy(name, state, memory, objects, params)

def _measure_obj_policy(state: State, memory: Dict,
                        objects: Sequence[Object],
                        params: Array) -> Action:
    name = "Measure"
    return _measure_policy(name, state, memory, objects, params)
...
_OPERATOR_NAME_TO_PARAM_SPACE = {
    ...
    "MoveToPlaceObject": Box(-np.inf, np.inf, (3, )),  # dx, dy, dyaw
    "MoveAwayFromObject": Box(-np.inf, np.inf, (2, )),  # rel dist, dyaw
    "MoveAwayFromObjectStair": Box(-np.inf, np.inf, (2, )),  # rel dist, dyaw
    ...
    "PlaceObjectInFront": Box(0, 1, (0, )),  # empty
    "Calibrate": Box(0, 1, (0, )),  # empty
    "Measure": Box(0, 1, (0, )),  # empty
}
...
_OPERATOR_NAME_TO_POLICY = {
    ...
    "MoveAwayFromObject": _move_away_from_object_policy,
    "HandViewObject": _hand_view_policy,
    "PickObjectFromTop": _pick_object_from_top_policy,
    "MoveToPlaceObject": _move_to_place_object_policy,
    "MoveToOnStairsHandViewObject": _move_to_on_stair_obj_policy,
    "MoveAwayFromObjectStair": _move_away_from_object_stair_policy,
    "PlaceObjectInFront": _place_object_in_front_policy,
    "Calibrate": _calibrate_obj_policy,
    "Measure": _measure_obj_policy,
}
...
class SpotViewPlanGroundTruthOptionFactory(GroundTruthOptionFactory):
    """Ground-truth options for the Spot View Planning environment."""
    ...
    def get_options(cls, env_name: str, types: Dict[str, Type],
        ...
        obj_type = types["target"]

        MoveToReach = _SpotParameterizedOption('MoveToReachObject', types=[robot_type, stair_type])
        Pick = _SpotParameterizedOption('PickObjectFromTop', types=[robot_type, stair_type])
        MoveToPlace = _SpotParameterizedOption('MoveToPlaceObject', types=[robot_type, stair_type, obj_type])
        Place = _SpotParameterizedOption('PlaceObjectInFront', types=[robot_type, stair_type, obj_type])
        ...
        MoveAwayFromObject = _SpotParameterizedOption('MoveAwayFromObject', types=[robot_type, obj_type])
        MoveAwayFromObjectStair = _SpotParameterizedOption('MoveAwayFromObjectStair', types=[robot_type, stair_type, obj_type])
        HandView = _SpotParameterizedOption('HandViewObject', types=[robot_type, obj_type])
        Calibrate = _SpotParameterizedOption('Calibrate', types=[robot_type, obj_type])
        Measure = _SpotParameterizedOption('Measure', types=[robot_type, obj_type])
        
        return {MoveToReach, Pick, MoveToPlace, Place, MoveToOnStairsHandView,
            MoveToHandView, MoveAwayFromObject, MoveAwayFromObjectStair, HandView, Calibrate, Measure}