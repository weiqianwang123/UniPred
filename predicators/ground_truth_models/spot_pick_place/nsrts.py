...
class SpotPickPlaceGroundTruthNSRTFactory(GroundTruthNSRTFactory):
    """Ground-truth NSRTs for the Spot View Planning environment."""
    ...
    def get_nsrts(env_name: str, types: Dict[str, Type],
        ...
        HoldingStair = predicates["HoldingStair"]
        HoldingTgt = predicates["HoldingTgt"]
        ...
        add_effects = {
            LiftedAtom(HoldingStair, [robot, stair]),
        }
        delete_effects = {
            LiftedAtom(Reachable, [robot, stair]),
            LiftedAtom(HandEmpty, [robot]),
        ...
        preconditions = {
            LiftedAtom(OnStair, [robot, stair]),
            LiftedAtom(HoldingTgt, [robot, obj]),
        }
        add_effects = {
            LiftedAtom(OnGround, [robot]),
            LiftedAtom(SurroundingClear, [robot]),
        ...
        delete_effects = {
            LiftedAtom(OnStair, [robot, stair]),
            LiftedAtom(Near, [stair, obj]), # This is needed..
        }
        ignore_effects = set()
        ...
        option = Put
        preconditions = {
            LiftedAtom(HoldingTgt, [robot, obj_inhand]),
            LiftedAtom(HandSees, [robot, obj_tgt]),
        }
        add_effects = {
