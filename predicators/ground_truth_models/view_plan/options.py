...
from predicators.spot_utils.utils_trivial import DEFAULT_HAND_DROP_OBJECT_POSE, \
    ...
    get_relative_se3_from_distance_and_angles
...
def _hand_view_object_policy(name: str, distance_param_idx: int,
    ...
    robot, localizer, _ = get_robot()
    robot_obj = objects[robot_obj_idx]
    hand_pose = utils.get_se3_hand_pose_from_state(state, robot_obj)
    ...
    hand_rel_pose = get_relative_se3_from_distance_and_angles(hand_pose, target_pose, 
                        distance, theta, phi)
    
    fn = move_hand_to_relative_pose
    fn_args = (robot, hand_rel_pose)
...
def _move_to_hand_view_object_policy(state: State, memory: Dict,
    ...
    min_distance = CFG.cam2obj_distance_tol_trivial[0] + CFG.spot_arm_length
    max_distance = CFG.cam2obj_distance_tol_trivial[1] + CFG.spot_arm_length
    yaw_param_idx = 1
    robot_obj_idx = 0
    target_obj_idx = 1
    do_gaze = False
...
def _move_away_from_object_policy(state: State, memory: Dict,
    ...
    distance_param_idx = 0
    min_distance = CFG.far_distance_trivial[0]
    max_distance = CFG.far_distance_trivial[1]
    yaw_param_idx = 1
    robot_obj_idx = 0
    target_obj_idx = 1
    do_gaze = False
...
def _hand_view_policy(state: State, memory: Dict,
    ...
    name = "HandViewObject"
    distance_param_idx = 0
    min_distance = CFG.cam2obj_distance_tol_trivial[0]
    max_distance = CFG.cam2obj_distance_tol_trivial[1]
    theta_param_idx = 1
    phi_param_idx = 2
    robot_obj_idx = 0
    target_obj_idx = 1
...
class ViewPlanGroundTruthOptionFactory(GroundTruthOptionFactory):
    """Ground-truth options for the Spot View Planning environment."""
    ...
    def get_options(cls, env_name: str, types: Dict[str, Type],
        ...
        return {MoveToHandView, MoveAwayFromObject, HandView, Calibrate, Measure}