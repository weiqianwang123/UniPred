...
def _hand_offset_sampler(state: State, robot_obj: Object,
    ...
    try:
        ...
        print("WARNING: Failed to find good hand pose sample.")
        # Pick distance and angle at random.
        abs_distance = rng.uniform(min_dist, max_dist)
...
def _move_to_hand_view_object_sampler(state: State, goal: Set[GroundAtom],
    ...
    min_dist = CFG.cam2obj_distance_tol_trivial[0] + CFG.spot_arm_length
    max_dist = CFG.cam2obj_distance_tol_trivial[1] + CFG.spot_arm_length
...
class ViewPlanGroundTruthNSRTFactory(GroundTruthNSRTFactory):
    """Ground-truth NSRTs for the Spot View Planning environment."""
    ...
    def get_nsrts(env_name: str, types: Dict[str, Type],
        ...
        # Predicates
        Viewable = predicates["Viewable"]
        ViewClear = predicates["ViewClear"]
        HandSees = predicates["HandSees"]
        NotBlocked = predicates["NotBlocked"]
        ...
        preconditions = {
            LiftedAtom(NotBlocked, [obj]),
            LiftedAtom(ViewClear, [robot]),
        }
        add_effects = {
            LiftedAtom(Viewable, [robot, obj]),
        }
        delete_effects = {
            LiftedAtom(ViewClear, [robot]),
        }
