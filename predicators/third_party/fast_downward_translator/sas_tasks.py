...
class SASTask:
    ...
    def dump(self):
        ...
        self.variables.dump()
        logging.info("%d mutex groups:" % len(self.mutexes))
        for mutex in self.mutexes:
            logging.info("group:")
            mutex.dump()
        logging.info("init:")
        ...
        logging.info("goal:")
        self.goal.dump()
        logging.info("%d operators:" % len(self.operators))
        for operator in self.operators:
            operator.dump()
        logging.info("%d axioms:" % len(self.axioms))
        for axiom in self.axioms:
            axiom.dump()
        logging.info("metric: %s" % self.metric)

    def output(self, stream):
        logging.info("begin_version", file=stream)
        logging.info(SAS_FILE_VERSION, file=stream)
        logging.info("end_version", file=stream)
        logging.info("begin_metric", file=stream)
        logging.info(int(self.metric), file=stream)
        logging.info("end_metric", file=stream)
        self.variables.output(stream)
        logging.info(len(self.mutexes), file=stream)
        for mutex in self.mutexes:
            mutex.output(stream)
        self.init.output(stream)
        self.goal.output(stream)
        logging.info(len(self.operators), file=stream)
        for op in self.operators:
            op.output(stream)
        logging.info(len(self.axioms), file=stream)
        for axiom in self.axioms:
            axiom.output(stream)
    ...
    def dump(self):
        for var, (rang,
            ...
            if axiom_layer != -1:
                ...
                axiom_str = ""
            logging.info("v%d in {%s}%s" % (var, list(range(rang)), axiom_str))

    def output(self, stream):
        logging.info(len(self.ranges), file=stream)
        for var, (rang, axiom_layer, values) in enumerate(
                zip(self.ranges, self.axiom_layers, self.value_names)):
            logging.info("begin_variable", file=stream)
            logging.info("var%d" % var, file=stream)
            logging.info(axiom_layer, file=stream)
            logging.info(rang, file=stream)
            assert rang == len(values), (rang, values)
            for value in values:
                logging.info(value, file=stream)
            logging.info("end_variable", file=stream)
...
class SASMutexGroup:
    ...
    def dump(self):
        for var, val in self.facts:
            logging.info("v%d: %d" % (var, val))

    def output(self, stream):
        logging.info("begin_mutex_group", file=stream)
        logging.info(len(self.facts), file=stream)
        for var, val in self.facts:
            logging.info(var, val, file=stream)
        logging.info("end_mutex_group", file=stream)
...
class SASInit:
    ...
    def output(self, stream):
        logging.info("begin_state", file=stream)
        for val in self.values:
            logging.info(val, file=stream)
        logging.info("end_state", file=stream)
...
class SASGoal:
    ...
    def output(self, stream):
        logging.info("begin_goal", file=stream)
        logging.info(len(self.pairs), file=stream)
        for var, val in self.pairs:
            logging.info(var, val, file=stream)
        logging.info("end_goal", file=stream)
    ...
    def dump(self):
        ...
        for var, pre, post, cond in self.pre_post:
            if cond:
                ...
                cond_str = ""
            logging.info("  v%d: %d -> %d%s" % (var, pre, post, cond_str))

    def output(self, stream):
        logging.info("begin_operator", file=stream)
        logging.info(self.name[1:-1], file=stream)
        logging.info(len(self.prevail), file=stream)
        for var, val in self.prevail:
            logging.info(var, val, file=stream)
        logging.info(len(self.pre_post), file=stream)
        for var, pre, post, cond in self.pre_post:
            logging.info(len(cond), end=' ', file=stream)
            for cvar, cval in cond:
                logging.info(cvar, cval, end=' ', file=stream)
            logging.info(var, pre, post, file=stream)
        logging.info(self.cost, file=stream)
        logging.info("end_operator", file=stream)
...
class SASAxiom:
    ...
    def output(self, stream):
        logging.info("begin_rule", file=stream)
        logging.info(len(self.condition), file=stream)
        for var, val in self.condition:
            logging.info(var, val, file=stream)
        var, val = self.effect
        logging.info(var, 1 - val, val, file=stream)
        logging.info("end_rule", file=stream)
