import logging
import predicators.third_party.fast_downward_translator.invariant_finder as invariant_finder
...
def choose_groups(groups, reachable_facts):
    ...
    while queue:
        ...
        result.append(group)
    logging.info(len(uncovered_facts), "uncovered facts")
...
def compute_groups(task, atoms, reachable_action_params):
    ...
    if DEBUG:
        for group in groups:
            if len(group) >= 2:
                logging.info("{%s}" % ", ".join(map(str, group)))
