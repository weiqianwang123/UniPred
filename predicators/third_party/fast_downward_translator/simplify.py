...
class DomainTransitionGraph:
    ...
    def dump(self):
        """Dump the DTG."""
        logging.info("DTG size:", self.size)
        logging.info("DTG init value:", self.init)
        logging.info("DTG arcs:")
        for source, destinations in sorted(self.arcs.items()):
            for destination in sorted(destinations):
                logging.info("  %d => %d" % (source, destination))
...
class VarValueRenaming:
    ...
    def dump(self):
        ...
        logging.info("variable count: %d => %d" % (old_var_count, self.new_var_count))
        logging.info("number of removed values: %d" % self.num_removed_values)
        logging.info("variable conversions:")
        for old_var_no, (new_var_no, new_values) in enumerate(
            ...
            if new_var_no is None:
                logging.info("variable %d [size %d] => removed" %
                      (old_var_no, old_size))
            else:
                new_size = self.new_sizes[new_var_no]
                logging.info("variable %d [size %d] => %d [size %d]" %
            ...
            for old_value, new_value in enumerate(new_values):
                ...
                logging.info("    value %d => %s" % (old_value, new_value))
        ...
        for var_no, values in enumerate(value_names):
            for value, value_name in enumerate(values):
                ...
                if new_value is always_true:
                    if DEBUG:
                        logging.info("Removed true proposition: %s" % value_name)
                elif new_value is always_false:
                    if DEBUG:
                        logging.info("Removed false proposition: %s" % value_name)
                else:
    ...
    def apply_to_operators(self, operators):
        ...
        for op in operators:
            ...
            if new_op is None:
                num_removed += 1
                if DEBUG:
                    logging.info("Removed operator: %s" % op.name)
            else:
                new_operators.append(new_op)
        logging.info("%d operators removed" % num_removed)
    ...
    def apply_to_axioms(self, axioms):
        ...
        for axiom in axioms:
            try:
                ...
                num_removed += 1
                if DEBUG:
                    logging.info("Removed axiom:")
                    axiom.dump()
            else:
                new_axioms.append(axiom)
        logging.info("%d axioms removed" % num_removed)
        axioms[:] = new_axioms
...
def filter_unreachable_propositions(sas_task):
    ...
    logging.info("%d propositions removed" % renaming.num_removed_values)
    if DEBUG:
        sas_task.validate()
