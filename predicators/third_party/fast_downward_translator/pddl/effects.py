...
class Effect:
    ...
    def dump(self):
        indent = "  "
        if self.parameters:
            logging.info("%sforall %s" %
                  (indent, ", ".join(map(str, self.parameters))))
            indent += "  "
        if self.condition != conditions.Truth():
            logging.info("%sif" % indent)
            self.condition.dump(indent + "  ")
            logging.info("%sthen" % indent)
            indent += "  "
        logging.info("%s%s" % (indent, self.literal))
    ...
    def _instantiate(self, var_mapping, init_facts, fluent_facts, result):
        ...
        try:
            ...
            return
        effects = []
        self.literal.instantiate(var_mapping, init_facts, fluent_facts,
                                 effects)
...
class UniversalEffect:
    ...
    def dump(self, indent="  "):
        logging.info("%sforall %s" % (indent, ", ".join(map(str, self.parameters))))
        self.effect.dump(indent + "  ")
...
class SimpleEffect:
    ...
    def dump(self, indent="  "):
        logging.info("%s%s" % (indent, self.effect))
