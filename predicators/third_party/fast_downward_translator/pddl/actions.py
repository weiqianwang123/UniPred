...
class Action:
    ...
    def dump(self):
        logging.info("%s(%s)" % (self.name, ", ".join(map(str, self.parameters))))
...
class PropositionalAction:
    ...
    def dump(self):
        ...
        for cond, fact in self.add_effects:
            logging.info("ADD: %s -> %s" % (", ".join(map(str, cond)), fact))
        for cond, fact in self.del_effects:
            logging.info("DEL: %s -> %s" % (", ".join(map(str, cond)), fact))
        logging.info("cost:", self.cost)
