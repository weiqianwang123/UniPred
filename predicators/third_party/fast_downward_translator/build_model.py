...
class JoinRule(BuildRule):
    ...
    def validate(self):
        ...
        right_vars = {
            v
            for v in right_args if isinstance(v, int) or v[0] == "?"
        }
        eff_vars = {v for v in eff_args if isinstance(v, int) or v[0] == "?"}
        assert left_vars & right_vars, self
        assert (left_vars
                | right_vars) == (left_vars & right_vars) | eff_vars, self
...
class ProductRule(BuildRule):
    ...
    def validate(self):
        ...
        cond_vars = [{
            v
            for v in cond.args if isinstance(v, int) or v[0] == "?"
        } for cond in self.conditions]
        all_cond_vars = reduce(set.union, cond_vars)
        eff_vars = {
            v
            for v in self.effect.args if isinstance(v, int) or v[0] == "?"
        }
...
class MatchGenerator:
    ...
    def dump(self, indent):
        ...
        for key in sorted(self.match_generator.keys()):
            logging.info("%sargs[%s] == %s:" % (indent, self.index, key))
...
def compute_model(prog):
    ...
    logging.info("Generated %d rules." % len(rules))
    ...
    logging.info("%d relevant atoms" % relevant_atoms)
    logging.info("%d auxiliary atoms" % auxiliary_atoms)
    logging.info("%d final queue length" % len(queue.queue))
    logging.info("%d total queue pushes" % queue.num_pushes)
    return queue.queue
