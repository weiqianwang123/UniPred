...
def add_effect(tmp_effect, result):
    ...
    [ConjunctiveEffect] [UniversalEffect] [ConditionalEffect]
    SimpleEffect.
    """
        ...
        if fact[0] == "=":
            ...
            if assignment.fluent in initial_assignments:
                ...
                if assignment.expression == prev.expression:
                    logging.info("Warning: %s is specified twice" % assignment,
...
def check_atom_consistency(atom,
    ...
    if atom in same_truth_value:
        if not atom_is_true:
            atom = atom.negate()
        logging.info("Warning: %s is specified twice in initial state specification" %
              atom)
