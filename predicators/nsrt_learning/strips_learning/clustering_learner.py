    ...
    def _learn(self) -> List[PNAD]:
        ...
        for segment in segments:
            if segment.has_option():
                ...
                if segment_param_option not in all_options:
                    all_options.add(segment_param_option)
        ...
        if CFG.strips_operator_eq_option:
            logging.info("Options == Operators, so learning operators w/ most data")
            # one option only has one operator, we learn it by using the 
            # pnad's num of segments
            final_pnads = []
            for option in all_options:
                pnads_for_option = [pnad for pnad in pnads if
                                    pnad.option_spec[0] == option]
                assert len(pnads_for_option) > 0, \
                    f"Option {option} has no associated PNADs"
                # find the pnad with the most segments
                max_pnad = max(pnads_for_option, key=lambda pnad: len(pnad.datastore))
                final_pnads.append(max_pnad)
            pnads = final_pnads
