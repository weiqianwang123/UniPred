...
def _learn_neural_sampler(datastores: List[Datastore], nsrt_name: str,
        ...
        for var in variables:
            ...
            if CFG.domain_sampler_data_filter is not None:
                ...
                if len(filtered_data) != len(data):
                    has_pcd = True
                    dropped_dim = len(data) - len(filtered_data)
            else:
                filtered_data = data
            X_classifier[-1].extend(filtered_data)
    ...
    if has_pcd:
        logging.warning(f"PCD: Dropped {dropped_dim} dimensions")
    X_arr_classifier = []
    ...
    classifier_weights = classifier.state_dict()
    save_path = utils.get_approach_load_path_str()
    weight_path = f"{save_path}_sampler_op_{nsrt_name}_cls_weight.pt"
    info_path = f"{save_path}_sampler_op_{nsrt_name}_cls_info.pt"
    classifier_info = {
        'input_shift': classifier._input_shift,
        'input_scale': classifier._input_scale,
        "x_dims": classifier._x_dims,
    }
    torch.save(classifier_weights, weight_path)
    torch.save(classifier_info, info_path)
    ...
    for state, sub, option, goal in positive_data:  # don't use negative data!
        ...
        for var in variables:
            ...
            if CFG.domain_sampler_data_filter is not None:
                ...
                filtered_data = data
            X_regressor[-1].extend(filtered_data)
        # for var in variables:
    ...
    regressor_weights = regressor.state_dict()
    regressor_info = {
        'input_shift': regressor._input_shift,
        'input_scale': regressor._input_scale,
        "output_shift": regressor._output_shift,
        "output_scale": regressor._output_scale,
        "x_dims": regressor._x_dims,
        "y_dim": regressor._y_dim,
    }
    info_path = f"{save_path}_sampler_op_{nsrt_name}_reg_info.pt"
    weight_path = f"{save_path}_sampler_op_{nsrt_name}_reg_weight.pt"
    torch.save(regressor_weights, weight_path)
    torch.save(regressor_info, info_path)
    # Construct and return sampler
    return _LearnedSampler(classifier, regressor, variables,
                           param_option).sampler

def _create_neural_sampler(variables: Sequence[Variable],
                          param_option: ParameterizedOption,
                          cls_weights: str,
                          cls_info: str,
                          reg_weights: str,
    ...
    n_reinitialize_tries=CFG.sampler_mlp_classifier_n_reinitialize_tries,
    weight_init="default")
    classifier_weights = torch.load(cls_weights)
    classifier_info = torch.load(cls_info)
    x_dims = classifier_info['x_dims']
    input_shift = classifier_info['input_shift']
    input_scale = classifier_info['input_scale']
    classifier.load(x_dims, input_shift, input_scale, classifier_weights)
    if CFG.sampler_learning_regressor_model == "neural_gaussian":
        regressor: DistributionRegressor = NeuralGaussianRegressor(
            seed=CFG.seed,
            hid_sizes=CFG.neural_gaus_regressor_hid_sizes,
    ...
    regressor_weights = torch.load(reg_weights)
    regressor_info = torch.load(reg_info)
    x_dims = regressor_info['x_dims']
    y_dim = regressor_info['y_dim']
    input_shift = regressor_info['input_shift']
    input_scale = regressor_info['input_scale']
    output_shift = regressor_info['output_shift']
    output_scale = regressor_info['output_scale']
    regressor.load(x_dims, y_dim, input_shift, input_scale, 
                   output_shift, output_scale,
                   regressor_weights)
    return _LearnedSampler(classifier, regressor, variables,
                           param_option).sampler
