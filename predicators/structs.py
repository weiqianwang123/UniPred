...
class DummyPredicate(Predicate):
    """Struct defining a dummy predicate with None classifier."""
    _classifier: Callable[[State, Sequence[Object]],
                          bool] = None
    ...
    def holds(self, state: State, objects: Sequence[Object]) -> bool:
        ...
        Performs type checking first.
        """
        raise NotImplementedError("This is Dummy Predicate, should not be called")

@dataclass(frozen=True, order=False, repr=False)
class NeuralPredicate(Predicate):
    """
    Struct defining a neural predicate with neural classifier.
    Tricky part is a neural classifier is not serializable, so we need to
    handle the save/load with care.
    """
    ...
    def __getstate__(self):
        ...
        state['_classifier_code'] = pkl.dumps(self._classifier.__code__)  # Serialize function code
        state['_classifier_globals'] = {k: v for k, v in self._classifier.__globals__.items() if k in self._classifier.__code__.co_names}
        state['_classifier_globals'] = pkl.dumps(state['_classifier_globals'])  # Serialize function globals
        
        # Serialize closure, filtering out non-pickleable elements
        closure = self._classifier.__closure__
        if closure:
            state['_classifier_closure'] = pkl.dumps([
                cell.cell_contents if self._is_pickleable(cell.cell_contents) else '__NON_PICKLEABLE__'
                for cell in closure
            ])
        else:
            state['_classifier_closure'] = None
        
        state.pop('_classifier')  # Remove the original _classifier
        return state

    def __setstate__(self, state):
        self.__dict__.update(state)
        code = pkl.loads(state['_classifier_code'])  # Deserialize function code
        globals_ = pkl.loads(state['_classifier_globals'])  # Deserialize function globals
        closure = pkl.loads(state['_classifier_closure']) if state['_classifier_closure'] else None
        
        if closure:
            closure = tuple(
                types.CellType(cell if cell != '__NON_PICKLEABLE__' else None)
                for cell in closure
            )
        
        # Bypass the frozen instance restriction
        object.__setattr__(self, '_classifier', types.FunctionType(code, globals_, name=code.co_name, argdefs=None, closure=closure))

    def set_classifier(self, classifier: Callable[[State, Sequence[Object]], bool]):
        # Bypass the frozen instance restriction
...
class LowLevelTask:
    ...
    def goal_holds(self, state: State) -> bool:
        """Return whether the goal of this task holds in the given state.
            The goal is a state, so we check that all the low-level features
        """
        for obj in self.goal:
            if not np.allclose(self.goal[obj], state[obj]):
                return False
        return True
...
DefaultLowLevelTask = LowLevelTask(DefaultState, DefaultState)
...
@dataclass(frozen=True, eq=False)
class LowLevelEnvironmentTask:
    """An initial observation and goal description.

    Environments produce environment tasks and agents produce and solve tasks.

    In fully observed settings, the init_obs will be a State and the
    goal_description will also be a State. For convenience, we can convert
    a LowLevelEnvironmentTask into a LowLevelTask in those cases.
    """
    init_obs: Observation
    goal_description: GoalDescription
    # See Task._alt_goal for the reason for this field.
    ...
    def task(self) -> LowLevelTask:
        ...
        # alt_goal field to.
        if self.alt_goal_desc is None:
            return LowLevelTask(self.init, self.goal)
        # If we turn the environment task into a task before replacing the goal
        # with the alternative goal, we have to set the task's alt_goal field
        # accordingly to leave open the possibility of doing that replacement
        # later.
        # Assumption: we currently assume the alternative goal description is
        # always a set of ground atoms.
        assert isinstance(self.alt_goal_desc, State)
        return LowLevelTask(self.init, self.goal, alt_goal=self.alt_goal_desc)
    ...
    def goal(self) -> Set[GroundAtom]:
        """Convenience method for environment tasks that are fully observed."""
        assert isinstance(self.goal_description, State)
        # assert not self.goal_description or isinstance(
        #     next(iter(self.goal_description)), GroundAtom)
        return self.goal_description
    ...
    def replace_goal_with_alt_goal(self) -> EnvironmentTask:
        ...
        """
        if self.alt_goal_desc is not None:
            return LowLevelEnvironmentTask(self.init_obs,
                                   goal_description=self.alt_goal_desc)
        return self


DefaultLevelEnvironmentTask = LowLevelEnvironmentTask(DefaultState, DefaultState)
...
@dataclass(frozen=True, repr=False, eq=False)
class LowLevelTrajectoryReward:
    """A structure representing a low-level trajectory, containing a state
    sequence, action sequence, and optional train task id. This trajectory may
    or may not be a demonstration.
    ...
    def __post_init__(self) -> None:
        assert len(self._states) == len(self._actions) + 1
        assert len(self._rewards) == len(self._actions)
        if self._is_demo:
            assert self._train_task_idx is not None
    ...
    def is_pos(self) -> bool:
        """Whether this trajectory is a positive demonstration."""
        return self._is_pos
...
@dataclass(repr=False, eq=False)
class MDPDataset:
    """A collection of LowLevelTrajectoryReward objects, and optionally, lists of
    annotations, one per trajectory.
    ...
    predicate classifiers deleted).
    """
    _trajectories: List[LowLevelTrajectoryReward]
    _annotations: Optional[List[Any]] = field(default=None)
    ...
    @property
    def trajectories(self) -> List[LowLevelTrajectoryReward]:
        """The trajectories in the dataset."""
        return self._trajectories
    ...
    def append(self,
               trajectory: LowLevelTrajectoryReward,
               annotation: Optional[Any] = None) -> None:
        """Append one more trajectory and annotation to the dataset."""
        if annotation is None:
            assert self._annotations is None
