...
from predicators.structs import Action, Dataset, MDPDataset, LowLevelTrajectory, \
    ParameterizedOption, State, Task, LowLevelTrajectoryReward


def create_demo_data(env: BaseEnv, train_tasks: List[Task],
                     known_options: Set[ParameterizedOption],
                     annotate_with_gt_ops: bool,
                     include_reward: bool=False) -> Dataset:
    ...
    if CFG.load_data:
        ...
        if include_reward:
            dataset = _generate_mdps(
                env,
                train_tasks,
                known_options,
                train_tasks_start_idx=0,
                annotate_with_gt_ops=annotate_with_gt_ops)
            logging.info(f"\n\nCREATED {len(dataset.trajectories)} MDPs for offline RL training")
        else:
...
def _generate_mdps(env: BaseEnv, train_tasks: List[Task],
                             known_options: Set[ParameterizedOption],
                             train_tasks_start_idx: int,
                             annotate_with_gt_ops: bool) -> Dataset:
    """Use the demonstrator to generate demonstrations, one per training task
    starting from train_tasks_start_idx."""
    assert CFG.demonstrator == "oracle", \
        "Only oracle approach is supported for generating MDPs."
    assert 'reward' in CFG.offline_data_method, \
        "Generating MDPs assumes reward-based data generation."
    # Instantiate CogMan with the oracle approach (to be used as the
    # demonstrator). This requires creating a perceiver and
    # execution monitor according to settings from CFG.
    options = get_gt_options(env.get_name())
    ...
    for idx, task in enumerate(train_tasks):
        ...
        try:                
            ...
            trajectories[idx] = {}
            rewards[idx] = {}
            # generate all the mdps and initial traj
            cogman.unset_override_policy()
            cogman.reset(env_task)
            all_mdps = cogman.get_all_mdps()
            pos_mdps[idx] = all_mdps['pos_mdp']
            neg_mdps[idx] = all_mdps['neg_mdp']

            for pos_idx in pos_mdps[idx].keys():
                logging.info(f"Generating MDP for task {idx} with pos idx {pos_idx}")
                option_plan, nsrt_plan, rw, metrics = pos_mdps[idx][pos_idx]
                ...
                assert task.goal_holds(traj.states[-1]), \
                    "Goal does not hold at the end of the pos trajectory."
                trajectories[idx][pos_idx] = traj
                rewards[idx][pos_idx] = rw

                if CFG.make_demo_videos:
                    assert video_monitor is not None
                    video = video_monitor.get_video()
                    outfile = f"{CFG.env}__{CFG.seed}__demo__task{idx}_pos{pos_idx}.mp4"
                    utils.save_video(outfile, video, rw.copy())
                if CFG.make_demo_images:
                    assert video_monitor is not None
                    video = video_monitor.get_video()
                    width = len(str(len(train_tasks)))
                    task_number = str(idx).zfill(width)
                    outfile_prefix = f"{CFG.env}__{CFG.seed}__demo__task{task_number}_pos{pos_idx}"
                    utils.save_images(outfile_prefix, video)
            
            for neg_idx in neg_mdps[idx].keys():
                logging.info(f"Generating MDP for task {idx} with neg idx {neg_idx}")
                option_plan, nsrt_plan, rw, metrics = neg_mdps[idx][neg_idx]
                policy = utils.option_plan_to_policy(option_plan)
                cogman.set_override_policy(policy)
                cogman.reset(env_task)
                ...
                assert not task.goal_holds(traj.states[-1]), \
                    "Goal holds at the end of the neg trajectory."
                trajectories[idx][neg_idx] = traj
                rewards[idx][neg_idx] = rw

                if CFG.make_demo_videos:
                    assert video_monitor is not None
                    video = video_monitor.get_video()
                    outfile = f"{CFG.env}__{CFG.seed}__demo__task{idx}_neg{neg_idx}.mp4"
                    # note, rw.copy() is used to prevent the reward in dictionary from being overwritten
                    utils.save_video(outfile, video, rw.copy())
                if CFG.make_demo_images:
                    assert video_monitor is not None
                    video = video_monitor.get_video()
                    width = len(str(len(train_tasks)))
                    task_number = str(idx).zfill(width)
                    outfile_prefix = f"{CFG.env}__{CFG.seed}__demo__task{task_number}_neg{neg_idx}"
                    utils.save_images(outfile_prefix, video)
    ...
    # Add is_demo flag and task index information into the trajectory.
    all_trajectories = []
    for task_idx in trajectories.keys():
        for traj_idx, traj in trajectories[task_idx].items():
            if traj_idx in pos_mdps[task_idx].keys():
                is_pos = True
            else:
                is_pos = False
            traj = LowLevelTrajectoryReward(traj.states,
                                            traj.actions,
                                            rewards[task_idx][traj_idx],
                                            is_pos,
                                            _is_demo=True,
    ...
    # if annotate_with_gt_ops:
    #     dataset = MDPDataset(trajectories, annotations)
    # else:
    dataset = MDPDataset(all_trajectories)
    return dataset
