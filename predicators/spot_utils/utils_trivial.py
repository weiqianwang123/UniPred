...
def get_viewable_geoms_for_nav(state: State) -> List[_Geom2D]:
    """Get all relevant viewable geometries for navigating."""
    viewable_geoms = []
    for obj in set(state):
        if obj.type == _target_object_type:
            x = state.get(obj, "x")
            y = state.get(obj, "y")
            viewable_region = utils.Circle(x, y, 2.0)
...
def sample_move_offset_from_target(
    ...
    allowed_regions: Collection[scipy.spatial.Delaunay],  # pylint: disable=no-member
    max_samples: int = 1000,
    obj_se2_pose: math_helpers.SE2Pose = None,
    ...
    for _ in range(max_samples):
        if not obj_rel:
            # sampled angle is relative to world frame, no consideration of obj rot
            ...
            distance = rng.uniform(min_distance, max_distance)
            d_angle = rng.uniform(min_angle, max_angle)
            obj_yaw = obj_se2_pose.angle
            angle = obj_yaw + d_angle
            dx = np.cos(angle) * distance
            dy = np.sin(angle) * distance
            x = obj_se2_pose.x + dx
            y = obj_se2_pose.y + dy
...
def get_relative_se3_from_distance_and_angles(
    ...
    rel_pose = hand_pose.inverse() * new_pose
