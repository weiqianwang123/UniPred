...
class YOLOSAM:
    def __init__(self, 
                 ...
                 sam_weights: str = 'sam.pt',
                 det_sim: float = 0.5,
                 det_conf: float = 0.1):
        from ultralytics import YOLO
        from ultralytics import FastSAM, SAM
        self.yolo = YOLO(yolo_weights)
        self.yolo.set_classes(["blue handel"])
        self.sam = SAM(sam_weights)
        self.det_conf = 0.5
        self.det_sim = det_sim
        self.det_conf = det_conf

    def filter_results(self, results):
        det_ids = results[0].boxes.data[:, 4] > self.det_conf
        results[0].boxes.data = results[0].boxes.data[det_ids]
        return results
    
    def detect_objects(self, rgbds: Dict[str, RGBDImageWithContext]):
        image_list = [Image.fromarray(rgbd.rotated_rgb).convert("BGR") \
                      for rgbd in rgbds.values()]
        det_results = self.yolo.predict(image_list, conf=self.det_sim, iou=0.1)
        det_results = self.filter_results(det_results)
        det_bbox = det_results[0].boxes.data[:, :4]
        seg_results = self.sam(image_list, bboxes=det_bbox, texts="blue")
        masks = seg_results[0]["masks"]
        masks = (masks * 255).astype(np.uint8).transpose(1, 2, 0)
        new_mask = ndimage.rotate(masks, -rgbd.image_rot, reshape=False)[:,:,0]
        # find out the middle point of the mask
        y_indices, x_indices = np.where(new_mask == 1)
        points = np.stack((x_indices, y_indices), axis=1)  # (x, y) pairs
        ...
        convex_center_x = hull_points[:, 0].mean()
        convex_center_y = hull_points[:, 1].mean()
        pixel = (int(convex_center_x), int(convex_center_y))
        return pixel
    
class LangSAMModel:
    def __init__(self, 
                 sam_type: str = 'sam2.1_hiera_small',
                 prompt: str = 'cyan grip'):
        from lang_sam import LangSAM
        self.model = LangSAM(sam_type=sam_type)
        self.prompt = prompt
    
    def segment(self, rgbds: Dict[str, RGBDImageWithContext]):
        image_list = []
        image_infos = {}
        i = 0
        for name, rgbd in rgbds.items():
            image_list.append(Image.fromarray(rgbd.rotated_rgb).convert("RGB"))
            image_infos[i] = {"camera_name": name, "image_rot": rgbd.image_rot,
                              "original_rgb": Image.fromarray(rgbd.rgb).convert("RGB")}
            i += 1
        for i, img in enumerate(image_list):
            img.save(f"image_{i}.png")
        prompts = [self.prompt] * len(image_list)
        results = self.model.predict(image_list, prompts,
                        box_threshold=0.3,
                        text_threshold=0.3)
        pixel_info = {}
        for i in range(len(rgbds.keys())):
            image_name = image_infos[i]["camera_name"]
            image_rot = image_infos[i]["image_rot"]
            original_rgb = image_infos[i]["original_rgb"]
            result = results[i]
            if len(result['labels']) == 0:
                logging.warning(f"No object detected in {image_name}")
                continue
            logging.info(f"Detected object in {image_name}: {results[0]['labels']}")
            masks = result["masks"]
            smallest_mask = np.ones_like(masks[0:1])
            for m in range(masks.shape[0]):
                mask = masks[m:m+1]
                if (mask.sum() < smallest_mask.sum()) and (mask.sum() > 300):
                    smallest_mask = mask
            new_mask = (smallest_mask * 255).astype(np.uint8).transpose(1, 2, 0)
            new_mask = ndimage.rotate(new_mask, -image_rot, \
                                      reshape=False)[:,:,0]
            # apply the new mask and save the image
            new_mask_image = Image.fromarray(new_mask)
            new_mask_image.save(f"mask_{i}.png")
            original_rgb.save(f"original_rgb_{i}.png")

            if smallest_mask.sum() > 4000:
                logging.warning(f"Mask too large in {image_name}")
                continue
            # find out the middle point of the mask
            y_indices, x_indices = np.where(new_mask >= 1)
            points = np.stack((x_indices, y_indices), axis=1)  # (x, y) pairs
            ...
            convex_center_y = hull_points[:, 1].mean()
            pixel = (int(convex_center_x), int(convex_center_y))
            pixel_info[image_name] = pixel
        
        # finally return the one with pixel closest to the image center
        closet_dist = 100000
        height, width = 480, 640
        best_info = None
        for name, pixel in pixel_info.items():
            dist = np.sqrt((pixel[0] - width / 2)**2 + (pixel[1] - height / 2)**2)
            if dist < closet_dist:
                closet_dist = dist
                best_info = {
                    name: pixel
...
class HumanLabeler:
    ...
    def segment(self, rgbds: Dict[str, 'RGBDImageWithContext']) -> Union[Dict[str, Tuple[int, int]], None]:
        image_list = []
        image_infos = {}
        i = 0
        ...
        # Prepare a single window with images
        display_width = 640
        display_height = 480
        combined_image = np.zeros((display_height, len(image_list) * display_width, 3), dtype=np.uint8)
        
        for idx, img in enumerate(image_list):
            resized_img = cv2.resize(np.array(img), (display_width, display_height))
            combined_image[:, idx * display_width:(idx + 1) * display_width, :] = resized_img

        # Display the images and collect human input
        pixel_info = {}
        selected_pixel = None
        selected_name = None
        ...
        def click_event(event, x, y, flags, param):
            nonlocal selected_pixel, selected_name, selected_image_index
            if event == cv2.EVENT_LBUTTONDOWN:
                # Identify which image was clicked
                image_idx = x // display_width
                if image_idx < len(image_list):
                    selected_image_index = image_idx
                    local_x = x % display_width
                    local_y = y
                    selected_pixel = (local_x, local_y)
                    selected_name = image_infos[image_idx]["camera_name"]
                    print(f"Selected {selected_name} at pixel {selected_pixel}")

        # Create OpenCV window and set mouse callback
        cv2.imshow("Click on an image and press Enter to confirm", combined_image)
        cv2.setMouseCallback("Click on an image and press Enter to confirm", click_event)
        ...
        if selected_pixel and selected_name:
            pixel_info[selected_name] = selected_pixel
            return pixel_info
        else:
...
def _get_pose_from_segmented_bounding_box(
    ...
    if len(segmented_depth) == 0:
        ...
        logging.info("WARNING: depth reading failed. Is hand occluding?")
        return None
...
def visualize_all_artifacts(artifacts: Dict[str,
    ...
    if flat_detections:
        ...
        logging.info(f"Wrote out to {detections_outfile}.")
        plt.close()
    ...
    if cameras_without_detections:
        ...
        logging.info(f"Wrote out to {no_detections_outfile}.")
        plt.close()
...
if __name__ == "__main__":
    ...
    def _run_manual_test() -> None:
        ...
        for obj_id, detection in detections.items():
            logging.info(f"Detected {obj_id} at {detection}")
    ...
    def _run_pythonic_bowl_test() -> None:
        ...
        def _detect_bowl(
            ...
            if blue_tape_found:
                roll = np.pi
                logging.info("Detected blue tape; bowl is upside-down!")
            else:
                roll = 0.0
                logging.info("Did NOT detect blue tape; bowl is right side-up!")
