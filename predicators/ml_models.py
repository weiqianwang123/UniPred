...
class NeuralGaussianRegressor(PyTorchRegressor, DistributionRegressor):
    ...
    def load(self, x_dims: Tuple[int], y_dim: int,
             input_shift: Array, input_scale: Array,
             output_shift: Array, output_scale: Array,
             model_weights: Dict[str, Tensor]) -> None:
        self._x_dims = x_dims
        self._y_dim = y_dim
        self._input_shift = input_shift
        self._input_scale = input_scale
        self._output_shift = output_shift
...
class MLPBinaryClassifier(PyTorchBinaryClassifier):
    ...
    def load(self, x_dims: Tuple[int], input_shift: Array, input_scale: Array,
             model_weights: Dict[str, Tensor]) -> None:
        self._x_dims = x_dims
        if len(model_weights) == 0:
            logging.warning("No model weights found. Do single class prediction.")
            self._do_single_class_prediction = True
            self._predicted_single_class = True
            return
        self._input_shift = input_shift
