...
from predicators.spot_utils.utils_pick_place import _target_pickplace_type, \
    _robot_hand_type, _stair_object_type, \
    DEFAULT_HAND2STAIR_TF, DEFAULT_STAIR2HAND_TF, \
    DEFAULT_OBJ2HAND_TF, DEFAULT_HAND2OBJ_TF, \
...
class PickPlaceEnv(BaseEnv):
    ...
    tgt_height_range: ClassVar[Tuple[float, float]] = (0.5, 0.7) # for tgt that doesn't need stairs
    high_tgt_height_range: ClassVar[Tuple[float, float]] = (0.9, 1.2)
    ...
    def __init__(self, use_gui: bool = True) -> None:
        ...
        self._HoldingStair = Predicate("HoldingStair", [self._spot_type, self._stairs_type],
                                    self._HoldingStair_holds)
        self._HoldingTgt = Predicate("HoldingTgt", [self._spot_type, self._tgt_obj_type],
                                    self._HoldingTarget_holds)
        ...
        self._OnStair = Predicate("OnStair", [self._spot_type, self._stairs_type],
                                    self._OnStair_holds)
        self._GraspingTgt = Predicate("GraspingTgt", [self._spot_type, \
                                        self._tgt_obj_type], \
                                        self._GraspingTgt_holds)
        self._GoalAchieved = Predicate("GoalAchieved", [self._tgt_obj_type, self._tgt_obj_type],
                                       self._GoalAchieved_holds)
    ...
    def predicates(self) -> Set[Predicate]:
        return {
            ...
            self._HoldingStair, self._AppliedTo, self._HandEmpty,
            self._GraspingTgt,
            self._HoldingTgt, self._Near, self._DirectViewable,
    ...
    def _generate_train_tasks(self) -> List[EnvironmentTask]:
        return self._get_tasks(num=CFG.num_train_tasks,
                               num_obj_lst=CFG.pickplace_num_obj_train,
                               num_stairs_lst=CFG.pickplace_num_stairs_train,
                               rng=self._train_rng,
                               test=False)
    ...
    def _generate_test_tasks(self) -> List[EnvironmentTask]:
        if CFG.in_domain_test:
            ...
            return self._get_tasks(num=CFG.num_test_tasks,
                                num_obj_lst=CFG.pickplace_num_obj_test,
                                num_stairs_lst=CFG.pickplace_num_stairs_test,
                                rng=self._test_rng,
                                test=True)
    ...
    def _get_tasks(self, num, num_obj_lst, num_stairs_lst, rng, test) \
        ...
        while task_id < num:
            ...
            if not skip:
                ...
                goal = set()
                for tgt in tgts:
                    if self._GraspingTgt.holds(init_state, [spot_robot, tgt]):
                        other_tgts = [t for t in tgts if t != tgt]
                        assert len(other_tgts) == 1
                        other_tgt = other_tgts[0]
                        goal.add(GroundAtom(self._GoalAchieved, [tgt, other_tgt]))
    ...
    def sample_tgt_using_arm(self, rng: np.random.Generator, 
        ...
        while True:
            ...
            if off_stair:
                ...
                tgt_x_axis_z = rng.uniform(0.9, 0.95)
            else:
                # spot is on stair
                # slightly face up
            ...
            hand_tgt_dist = (CFG.pickplace_cam2obj_distance_tol[0] + CFG.pickplace_cam2obj_distance_tol[1]) / 2
            ...
            dx_max = CFG.pickplace_viewobj_distance[1] * np.cos(tgt_yaw)
            dx_min = CFG.pickplace_viewobj_distance[0] * np.cos(tgt_yaw)
            dy_max = CFG.pickplace_viewobj_distance[1] * np.sin(tgt_yaw)
            dy_min = CFG.pickplace_viewobj_distance[0] * np.sin(tgt_yaw)
            dx = [dx_min, dx_max]
            dy = [dy_min, dy_max]
            dtheta = CFG.pickplace_move2hand_view_yaw_tol
    ...
    def render_state_plt(self,
        ...
        for tgt in state.get_objects(self._tgt_obj_type):
            ...
            name2info[tgt.name] = {
                'color': list(color),
                'achieved': state.get(tgt, "achieved")
            }
        ...
        labels.append(f"Spot - Grasped: {robot_calibrated}")
        for name, info in name2info.items():
            ...
            patches.append(color_patch)
            if 'tgt' in name:
                labels.append(f"{name} - Achieved: {info['achieved']}")
            else:
    ...
    def _HandSees_holds(self, state: State, objects: Sequence[Object]) -> bool:
        spot, obj = objects
    ...
    def _Viewable_Arm_holds(self, state: State, objects: Sequence[Object]) -> bool:
        spot, obj = objects
        object_held = state.get(obj, "held")
        # held object is not viewable
        if object_held > 0.5:
            return False
    ...
    def _HoldingStair_holds(self, state: State, objects: Sequence[Object]) -> bool:
        spot, obj = objects
        if _handempty_classifier(state, [spot]):
            return False
        return state.get(obj, "held") > 0.5

    def _HoldingTarget_holds(self, state: State, objects: Sequence[Object]) -> bool:
        spot, obj = objects
        if _handempty_classifier(state, [spot]):
            return False
        return state.get(obj, "held") > 0.5
    ...
    def _GoalAchieved_holds(self, state: State, objects: Sequence[Object]) -> bool:
        ...
        if obj1 == obj2:
            return False
        both_achieved = state.get(obj1, "achieved") and state.get(obj2, "achieved")
        on = (state.get(obj1, "z") - state.get(obj2, "z") > 0.05)
        return (both_achieved and on)
    
    def _GraspingTgt_holds(self, state: State, objects: Sequence[Object]) -> bool:
        spot = objects[0]
        tgt = objects[1]
    ...
    def simulate(self, state: State, action: Action) -> State:
        ...
        if action_name == "MoveToHandViewObject" or \
            action_name == "MoveAwayFromOnStair" or \
            action_name == "MoveAwayFromOffStair" or \
            action_name == "MoveToReachObject" or \
            action_name == "MoveToPlaceStair":
            spot = action_objs[0]
            ...
            for stair_obj in stair_objs:
                if self._HoldingStair_holds(state, [spot, stair_obj]):
                    ...
                    break
            tgt_objs = state.get_objects(self._tgt_obj_type)
            for tgt_obj in tgt_objs:
                if self._HoldingTarget_holds(state, [spot, tgt_obj]):
                    # Assume the tgt pose is relatively fixed to the spot pose
                    new_tgt_pose = new_hand_pose.mult(DEFAULT_HAND2OBJ_TF)
                    next_state.set(tgt_obj, "x", new_tgt_pose.x)
                    next_state.set(tgt_obj, "y", new_tgt_pose.y)
                    next_state.set(tgt_obj, "z", new_tgt_pose.z)
                    next_state.set(tgt_obj, "qx", new_tgt_pose.rot.x)
                    next_state.set(tgt_obj, "qy", new_tgt_pose.rot.y)
                    next_state.set(tgt_obj, "qz", new_tgt_pose.rot.z)
                    next_state.set(tgt_obj, "qw", new_tgt_pose.rot.w)
                    break
            if action_name == "MoveToPlaceStair":
                spot, stair, _ = action_objs
                if not self._HoldingStair_holds(next_state, [spot, stair]):
                    return next_state
            ...
            if not self._GraspingTgt_holds(next_state, [spot, obj]):
                return next_state
            if not self._HandEmpty.holds(next_state, [spot]):
                return next_state
            next_state.set(obj, "held", 1.0)
            spot_pose = utils.get_se3_pose_from_state(next_state, spot)
            new_hand_pose_mat = self.spot_kinematics.compute_fk(spot_pose.to_matrix(), \
                                DEFAULT_HAND_STOW_ANGLES)[-1]
            new_hand_pose = math_helpers.SE3Pose.from_matrix(new_hand_pose_mat)
            new_hand_pose = add_noise_to_pose(new_hand_pose, self._train_rng)
            next_state.set(spot, "ee_x", new_hand_pose.x)
            ...
            next_state.set(spot, "ee_qw", new_hand_pose.rot.w)
            next_state.set(spot, "gripper_open_percentage", 0.0)
            new_obj_pose = new_hand_pose.mult(DEFAULT_HAND2OBJ_TF)
            next_state.set(obj, "x", new_obj_pose.x)
            next_state.set(obj, "y", new_obj_pose.y)
            next_state.set(obj, "z", new_obj_pose.z)
            next_state.set(obj, "qx", new_obj_pose.rot.x)
            next_state.set(obj, "qy", new_obj_pose.rot.y)
            next_state.set(obj, "qz", new_obj_pose.rot.z)
            next_state.set(obj, "qw", new_obj_pose.rot.w)
            ...
            spot, obj_inhand, obj_tgt = action_objs
            if not self._HandSees.holds(next_state, [spot, obj_tgt]):
                return next_state
            # then check if the spot is calibrated
            if not self._HoldingTarget_holds(next_state, [spot, obj_inhand]):
                return next_state
            # finally, set the measured flag
            next_state.set(obj_inhand, "achieved", 1)
            next_state.set(obj_tgt, "achieved", 1)
            next_state.set(spot, "gripper_open_percentage", 1.0)
            return next_state
            ...
            for stair_obj in stair_objs:
                if self._HoldingStair_holds(state, [spot, stair_obj]):
                    # can't hand view with stair in hand
                    return next_state
            # update the EE pose
...
class SpotPickPlaceEnv(PickPlaceEnv):
    def __init__(self, use_gui: bool = True) -> None:
        super().__init__(use_gui)
    ...
    def _build_observation(self,
        ...
        while (body_error_trans > 0.18 and (body_rot_y > -0.05)):
    ...
    def _get_tasks(self, spot_home_pose, \
        ...
        while task_id < num:
            ...
            if num_stairs == 1:
                ...
                for i, stair in enumerate(stairs):
                    if i == 0:
                        # first stair is always the nearest one, applied to the tgt
                        pos_id = min_dis_idx
                    else:
                        pos_id = 1 - min_dis_idx
                    r, g, b = self.all_colors[color_id]
                    color_id += 1
                    state_dict[stair] = {
                        "x": pos[pos_id][0], "y": pos[pos_id][1], "z": stair_height + self.ground_z,
                        "qx": rot[pos_id][0], "qy": rot[pos_id][1], "qz": rot[pos_id][2], "qw": rot[pos_id][3],
                        "shape": 1,
                        "height": stair_height,
                        "width": self.stair_sz,
                        "length": self.stair_sz,
            ...
            while True:
                ...
                if (not (geom1.intersects(geom2)) and \
                    ...
                    if num_stairs == 1:
                        # second object doesn't need the stair
            ...
            if num_stairs == 1:
                ...
                # always make the nearest stair the first stair for grasping tgt
                stair_id = [3, 2]
            ...
            if not skip:
                ...
                goal = set()
                for tgt in tgts:
                    if self._GraspingTgt.holds(init_state, [self._spot_object, tgt]):
                        other_tgts = [t for t in tgts if t != tgt]
                        assert len(other_tgts) == 1
                        other_tgt = other_tgts[0]
                        goal.add(GroundAtom(self._GoalAchieved, [tgt, other_tgt]))
    ...
    def _HandEmpty_holds(self, state: State, objects: Sequence[Object]) -> bool:
        robot = objects[0]
        for stair in state.get_objects(self._stairs_type):
            if self._HoldingStair_holds(state=state, objects=[robot, stair]):
                return False
        for tgt in state.get_objects(self._tgt_obj_type):
            if self._HoldingTarget_holds(state=state, objects=[robot, tgt]):
                return False
        return True
    ...
    def _Near_holds(self, state: State, objects: Sequence[Object]) -> bool:
        ...
        stair_pose = utils.get_se3_pose_from_state(state, stair)
        obj_pose = utils.get_se3_pose_from_state(state, tgt)
        # hack this just use distance
        dist_2d = np.sqrt((stair_pose.x - obj_pose.x)**2 + (stair_pose.y - obj_pose.y)**2)
        if dist_2d < 0.8:
            return True
        # foot pose on stair
        # body_pose_on_stair = stair_pose * DEFAULT_STAIR2BODY_ONSTAIR_TF
    ...
    def state2obs(self, state: State) -> _SpotArmObservation:
        ...
        for obj in state.get_objects(self._tgt_obj_type):
            ...
            objects_in_view[obj]["achieved"] = state.get(obj, "achieved")
        ...
        for stair in state.get_objects(self._stairs_type):
            ...
            objects_in_view[stair]["rgb"] = (state.get(stair, "r"),
                                             state.get(stair, "g"),
                                             state.get(stair, "b"))
            if state.get(stair, "held") and object_in_hand is None:
                object_in_hand = stair
            elif state.get(stair, "held") and object_in_hand is not None:
                raise ValueError("Two objects are held at the same time!")
    ...
    def obs2state(self, obs: _SpotArmObservation) -> State:
        ...
        for obj, info in obs.objects_in_view.items():
            ...
            elif obj.is_instance(self._tgt_obj_type):
                held_val = 1.0 if obj == obs.object_in_hand else 0.0
                state_dict[obj]["held"] = held_val
