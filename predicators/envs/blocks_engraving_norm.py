...
from predicators.envs.blocks_engraving import BlocksEngraveVecEnv
...
class BlocksEngravePcdNormEnv(BlocksEngraveVecEnv):
    ...
    def _gen_pcd_for_block(self, irr_x: float, irr_y: float, irr_h: float,
        ...
        pc_top, norms_top = sample_points_from_meshes(
            top_mesh, 
            num_samples=num_top, 
            return_normals=True
        )
        pc_top = pc_top.squeeze(0).cpu().numpy()
        norms_top = norms_top.squeeze(0).cpu().numpy()

        # Sample points + normals from the rest
        pc_rest, norms_rest = sample_points_from_meshes(
            cube_minus_top_mesh, 
            num_samples=n_rest, 
            return_normals=True
        )
        pc_rest = pc_rest.squeeze(0).cpu().numpy()
        norms_rest = norms_rest.squeeze(0).cpu().numpy()

        # Combine
        pcd = np.concatenate([pc_top, pc_rest], axis=0)
        nmd = np.concatenate([norms_top, norms_rest], axis=0)

        # Optional: translate points but not normals
        pcd -= self._block_size / 2

        # If desired, store them together
        pcd_with_normals = np.concatenate([pcd, nmd], axis=-1)  # shape (N, 6)
    ...
    def _transform_pcd(
        ...
        """
        Applies a transformation to the block's point cloud (and optionally normals):
        1) Optionally rotate by 180 deg about the Y-axis if face_up == 0.0
        2) Translate to (x,y,z)
        
        Parameters
        ----------
        pcd : np.ndarray
            Shape (N, 3). The point coordinates.
        x, y, z : float
            The translation amounts in each dimension.
        face_up : float
            If 0.0, rotate 180 deg about Y; else no rotation.
        normals : np.ndarray, optional
            Shape (N, 3). The normals corresponding to each point. 
            If provided, will be rotated by the same rotation matrix (no translation).

        Returns
        -------
        If normals is None:
            pcd_transformed : (N, 3) array.
        Else:
            (pcd_transformed, normals_transformed) : both (N, 3).
        """
        # 1) Determine rotation (180 deg about Y if face_up == 0.0)
        if face_up == 0.0:
            # Double-check that from_pitch(np.pi) indeed yields a 180 deg rotation about Y in your codebase
        ...
        # 2) Build the full 4×4 transformation matrix for points
        trans_mat = np.eye(4, dtype=pcd.dtype)
        trans_mat[:3, :3] = rot_mat
        trans_mat[:3, 3] = np.array([x, y, z], dtype=pcd.dtype)

        # 3) Convert points to homogeneous coords (x, y, z, 1)
        ones = np.ones((pcd.shape[0], 1), dtype=pcd.dtype)
        pcd_hom = np.hstack([pcd, ones])  # (N, 4)

        # 4) Apply transformation to points
        pcd_transformed_hom = pcd_hom @ trans_mat.T  # (N, 4)
        pcd_transformed = pcd_transformed_hom[:, :3] # (N, 3)

        # 5) If normals are provided, apply only the rotation (no translation)
        # Just rotate the normals:
        # normals_transformed = normals × R^T
        # or (normals @ rot_mat.T), depending on your convention
        normals_transformed = normals @ rot_mat.T

        return pcd_transformed, normals_transformed
    ...
    def _sample_state_from_piles(self, piles: List[List[Object]],
        ...
        for block, pile_idx in block_to_pile_idx.items():
            ...
            pcd_norm = self._gen_pcd_for_block(eng_x, eng_y, eng_h, sigma)
            pcd = pcd_norm[:, :3]
            normals = pcd_norm[:, 3:]
            pcd_transformed, normals_transformed = self._transform_pcd(pcd, normals, x, y, z, face_up)
            state_dict[block] = {
                "pcd": pcd_transformed,
                "norm": normals_transformed,
                "pose_x": x,
                "pose_y": y,
                "pose_z": z,
                "color_r": r,
    ...
    def _transition_engrave(self, state: State, x: float, y: float, z: float,
        ...
        sigma = self._train_rng.uniform(CFG.blocks_engrave_sigma[0],
                                        CFG.blocks_engrave_sigma[1])
        new_pcd_norm = self._gen_pcd_for_block(eng_x, eng_y, eng_h, sigma)
        ...
        new_pcd = new_pcd_norm[:, :3].copy()
        new_norms = new_pcd_norm[:, 3:].copy()
        pcd_transformed, normals_transformed = self._transform_pcd(new_pcd, new_norms, top_block_x, \
                            top_block_y, top_block_z, face_up)
        next_state.set(top_block, "pcd", pcd_transformed.copy())
        next_state.set(top_block, "norm", normals_transformed.copy())
        ...
        new_pcd_norm = self._gen_pcd_for_block(mirrored_x, eng_y, -eng_h, sigma)
        ...
        new_pcd = new_pcd_norm[:, :3].copy()
        new_norms = new_pcd_norm[:, 3:].copy()
        pcd_transformed, normals_transformed = self._transform_pcd(new_pcd, new_norms, bottom_block_x, \
                            bottom_block_y, bottom_block_z, face_up)
        next_state.set(bottom_block, "pcd", pcd_transformed.copy())
        next_state.set(bottom_block, "norm", normals_transformed.copy())
        return next_state
    ...
    def _transition_rotate(self, state: State, x: float, y: float, z: float,
        ...
        # Update the normals
        normals = state.get(block, "norm").copy()
        normals_rotated = normals @ rot_mat.T
        next_state.set(block, "norm", normals_rotated)
    ...
    def _transition_pick(self, state: State, x: float, y: float,
        ...
        # change pcd, no need to change normals
