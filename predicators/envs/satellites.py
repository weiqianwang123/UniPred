...
class SatellitesEnv(BaseEnv):
    ...
    id_tol: ClassVar[float] = 0.1 # for model predicting error
    ...
    def render_state_plt(
        ...
        for obj in state.get_objects(self._obj_type):
            ...
            if self._HasChemX_holds(state, [obj]) and self._HasChemY_holds(state, [obj]):
                ...
                legend_info.append(f"Object {int(state.get(obj, 'id'))}: HasX+HasY")
    ...
    def _Sees_holds(self, state: State, objects: Sequence[Object]) -> bool:
        ...
        # We end up not checking occlusion as it is not modeled at the high level.
        # Maybe we should also apply Nishanth's work in the future on top of ours.
...
class SatellitesHardEnv(SatellitesEnv):
    """A hard version of the SatellitesEnv that has more objects and satellites.
       The satellites also have a feature indicating whether its view is clear.
       This allows us to learn all the predicates with the assumption that the 
       predicates are a function of only their argument's states.
       I.e., no higher-order predicates.
    """
